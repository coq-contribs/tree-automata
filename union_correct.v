...
Proof.
	...
	simpl in H0. elim (H a a0). intros. split with x. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H2 in H0. inversion H0. elim H1. intros. rewrite (ad_eq_complete _ _ H2) in H3. split. exact H3. exact (upl_conv_0_correct_wrt_sign_invar _ _ H5). inversion H0. simpl in |- *. rewrite (ad_eq_correct a). 
	...
	elim (H sigma (pre_ad_O pa)) with (a := ad_z) (p := p). intros.
	split with x. simpl in H3. exact H3. intros. elim (H1 (ad_double a) p0). intros. split with x. simpl in |- *. exact H4. induction  a as [| p1]; simpl in |- *; exact H3. exact H2. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. elim (H0 sigma (pre_ad_I pa)) with (a := ad_x p0) (p := p). intros. split with x. simpl in H3. exact H3.
	intros. elim (H1 (ad_double_plus_un a) p1). intros. split with x.
	...
	elim (H sigma (pre_ad_O pa)) with (a := ad_x p0) (p := p). intros.
	split with x. simpl in H3. exact H3. intros. elim (H1 (ad_double a) p1). intros. split with x. induction  a as [| p2]; simpl in H4; simpl in |- *; exact H4. induction  a as [| p2]; simpl in |- *; exact H3. simpl in H2. exact H2.
	elim (H0 sigma (pre_ad_I pa)) with (a := ad_z) (p := p). intros. split with x. simpl in H3. exact H3. intros. elim (H1 (ad_double_plus_un a) p0). intros. split with x. induction  a as [| p1]; simpl in |- *; simpl in H4; exact H4. induction  a as [| p1]; simpl in |- *; exact H3. simpl in H2. exact H2.
Qed.
...
Proof.
	...
	unfold state_correct_wrt_sign_with_offset in H.
	unfold state_correct_wrt_sign_with_offset in |- *. intros.
	simpl in H0. elim (H a a0). intros. split with x. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H2 in H0. inversion H0. elim H1. intros. rewrite (ad_eq_complete _ _ H2) in H3. split. exact H3. exact (upl_conv_1_correct_wrt_sign_invar _ _ H5). inversion H0. simpl in |- *. rewrite (ad_eq_correct a). 
	reflexivity. intros. unfold state_correct_wrt_sign_with_offset in H1.
	unfold state_correct_wrt_sign_with_offset in |- *. intros. unfold state_correct_wrt_sign_with_offset in H. unfold state_correct_wrt_sign_with_offset in H0. induction  a as [| p0]. simpl in H2.
	elim (H sigma (pre_ad_O pa)) with (a := ad_z) (p := p). intros.
	split with x. simpl in H3. exact H3. intros. elim (H1 (ad_double a) p0). intros. split with x. simpl in |- *. exact H4. induction  a as [| p1]; simpl in |- *; exact H3. exact H2. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. elim (H0 sigma (pre_ad_I pa)) with (a := ad_x p0) (p := p). intros. split with x. simpl in H3. exact H3.
...
Proof.
	...
	intros. simpl in H0. elim (bool_is_true_or_false (ad_eq a a1)); intros. rewrite H1 in H0. inversion H0. apply (umpl_conv_0_correct_wrt_sign_invar a0 sigma). apply (H a a0).
	simpl in |- *. rewrite (ad_eq_correct a). reflexivity. rewrite H1 in H0.
	inversion H0. intros. simpl in H2. induction  a as [| p]. apply (H sigma) with (a := ad_z) (s := s). intros. apply (H1 (ad_double a) s0). 
	induction  a as [| p]; simpl in |- *; exact H3. exact H2. induction  p as [p Hrecp| p Hrecp| ]; simpl in H2.
	apply (H0 sigma) with (a := ad_x p) (s := s). intros. apply (H1 (ad_double_plus_un a) s0). induction  a as [| p0]; simpl in |- *; exact H3. exact H2.
	apply (H sigma) with (a := ad_x p) (s := s). intros. apply (H1 (ad_double a) s0). induction  a as [| p0]; simpl in |- *; exact H3. exact H2. apply (H0 sigma) with (a := ad_z) (s := s). intros. apply (H1 (ad_double_plus_un a) s0).
...
Proof.
	...
	induction  a as [| p]. simpl in H0. exact (udta_conv_0_correct_wrt_sign_invar_0 d sigma H ad_z s H0). induction  p as [p Hrecp| p Hrecp| ]; simpl in H0. inversion H0.
	exact (udta_conv_0_correct_wrt_sign_invar_0 d sigma H (ad_x p) s H0).
	inversion H0.
Qed.
...
Proof.
	unfold predta_correct_wrt_sign in |- *. simple induction d. intros. inversion H0.
	intros. simpl in H0. elim (bool_is_true_or_false (ad_eq a a1)); intros. rewrite H1 in H0. inversion H0. apply (umpl_conv_1_correct_wrt_sign_invar a0 sigma). apply (H a a0).
	simpl in |- *. rewrite (ad_eq_correct a). reflexivity. rewrite H1 in H0.
	inversion H0. intros. simpl in H2. induction  a as [| p]. apply (H sigma) with (a := ad_z) (s := s). intros. apply (H1 (ad_double a) s0). 
	induction  a as [| p]; simpl in |- *; exact H3. exact H2. induction  p as [p Hrecp| p Hrecp| ]; simpl in H2.
	apply (H0 sigma) with (a := ad_x p) (s := s). intros. apply (H1 (ad_double_plus_un a) s0). induction  a as [| p0]; simpl in |- *; exact H3. exact H2.
...
Proof.
	...
	exact (udta_conv_1_correct_wrt_sign_invar_0 d sigma H (ad_x p) s H0).
	inversion H0. exact (udta_conv_1_correct_wrt_sign_invar_0 d sigma H ad_z s H0).
Qed.
...
Proof.
	...
	intros. simpl in H1. elim (bool_is_true_or_false (ad_eq a1 a)); intros; rewrite H2 in H1. elim (H a a0). intros. elim (H0 a p).
	intros. elim H3. elim H4. intros. simpl in H1. elim (bool_is_true_or_false (ad_eq a1 a2)); intros; rewrite H9 in H1;
  inversion H1. rewrite H7 in H5. inversion H5. split with x.
	rewrite <- H12 in H6. split. rewrite <- (ad_eq_complete _ _ H9). rewrite (ad_eq_complete _ _ H2). exact H7. exact (union_pl_correct_wrt_sign_invar p a0 x H6 H8). simpl in |- *. rewrite H2. reflexivity. simpl in |- *. rewrite (ad_eq_correct a). reflexivity. elim (ad_sum (ad_xor a a1)).
	intro y. elim y. intros x y0. rewrite y0 in H1. rewrite (MapPut1_semantics prec_list x a a1 a0 p y0) in H1. elim (bool_is_true_or_false (ad_eq a a2)); intros; rewrite H3 in H1. inversion H1. elim (H a a0). intros.
	elim H4. intros. split with x0. rewrite <- (ad_eq_complete _ _ H3).
	rewrite <- H5. exact H4. simpl in |- *. rewrite (ad_eq_correct a).
	reflexivity. elim (bool_is_true_or_false (ad_eq a1 a2)); intros.
	rewrite H4 in H1. inversion H1. elim (H0 a1 p). intros. split with x0. rewrite (ad_eq_complete _ _ H4) in H5. rewrite H6 in H5.
	exact H5. simpl in |- *. rewrite (ad_eq_correct a1). reflexivity.
	rewrite H4 in H1. inversion H1. intros y. rewrite (ad_eq_comm a1 a) in H2. rewrite (ad_xor_eq_true _ _ y) in H2. inversion H2.
  ...
  (state_correct_wrt_sign_with_offset (M1 prec_list ad_z p) sigma
     (pre_ad_O pa)). intros. induction  a0 as [| p1].
	elim (H _ _ _ _ H5 H6 ad_z p0). intros. split with x. simpl in H7.
	exact H7. exact H3. induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]. elim (H4 (ad_x p1) p0). intros.
	split with x. simpl in H7. exact H7. exact H3. elim (H _ _ _ _ H5 H6 (ad_x p1) p0). intros. split with x. simpl in H7. exact H7.
	exact H3. elim (H4 ad_z p0). intros. split with x. simpl in H7.
	exact H7. exact H3. unfold state_correct_wrt_sign_with_offset in |- *.
	intros. elim (H2 (ad_double a) p1). intros. split with x.
	induction  a as [| p2]; simpl in |- *; simpl in H7; exact H7. simpl in H6. 
	elim (ad_sum a). intros y. elim y. intros x y0. rewrite y0 in H6.
	inversion H6. intros y. rewrite y in H6. rewrite y. simpl in |- *.
	exact H6. induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]. cut
  (state_correct_wrt_sign_with_offset (M1 prec_list (ad_x p1) p) sigma
     (pre_ad_I pa)). intro. induction  a0 as [| p2]. simpl in H3. elim (H5 ad_z p0). intros. split with x. simpl in H7. exact H7. exact H3. induction  p2 as [p2 Hrecp2| p2 Hrecp2| ]; simpl in H3. elim (H0 _ _ _ _ H4 H6 (ad_x p2) p0). intros. split with x. simpl in H7.
	exact H7. exact H3. elim (H5 (ad_x p2) p0). intros. split with x.
	simpl in H7. exact H7. exact H3. elim (H0 _ _ _ _ H4 H6 ad_z p0).
	intros. split with x. simpl in H7. exact H7. exact H3. 
	unfold state_correct_wrt_sign_with_offset in |- *. intros. elim (H2 (ad_double_plus_un a) p2). intros. split with x. induction  a as [| p3]; simpl in |- *; simpl in H7; exact H7. induction  a as [| p3]; simpl in |- *; exact H6. cut
  (state_correct_wrt_sign_with_offset (M1 prec_list (ad_x p1) p) sigma
     (pre_ad_O pa)). intros. induction  a0 as [| p2]. simpl in H3.
	elim (H _ _ _ _ H5 H6 ad_z p0). intros. split with x. simpl in H7.
	exact H7. exact H3. induction  p2 as [p2 Hrecp2| p2 Hrecp2| ]; simpl in H3. elim (H4 (ad_x p2) p0).
	intros. split with x. simpl in H7. exact H7. exact H3. elim (H _ _ _ _ H5 H6 (ad_x p2) p0). intros. split with x. simpl in H7.
	exact H7. exact H3. elim (H4 ad_z p0). intros. split with x. simpl in H7. exact H7. exact H3. unfold state_correct_wrt_sign_with_offset in |- *.
	intros. elim (H2 (ad_double a) p2). intros. split with x. 
  ...
  (state_correct_wrt_sign_with_offset (M1 prec_list ad_z p) sigma
	...
	elim (H5 ad_z p0). intros. split with x. simpl in H7. exact H7.
	exact H3. induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]; simpl in H3. elim (H0 _ _ _ _ H4 H6 (ad_x p1) p0). intros. split with x. simpl in H7. exact H7.
	exact H3. elim (H5 (ad_x p1) p0). intros. split with x. simpl in H7. exact H7. exact H3. elim (H0 _ _ _ _ H4 H6 ad_z p0).
	intros. split with x. simpl in H7. exact H7. exact H3. unfold state_correct_wrt_sign_with_offset in |- *. intros. elim (H2 (ad_double_plus_un a) p1). intros. split with x. induction  a as [| p2]; simpl in |- *; simpl in H7; exact H7. induction  a as [| p2]; simpl in |- *. simpl in H6. exact H6. simpl in H6. inversion H6. unfold state_correct_wrt_sign_with_offset in |- *.
	intros. elim (H1 (ad_double_plus_un a1) p1). intros. split with x.
	...
	elim (H1 (ad_double a1) p1). intros. split with x. induction  a1 as [| p2]; simpl in |- *; simpl in H5; exact H5. induction  a1 as [| p2]; simpl in |- *; exact H4.
Qed.
...
Proof.
	...
	simpl in |- *. simple induction s1. intros. exact (union_mpl_correct_wrt_sign_invar_0 (M0 prec_list) a a0 pa sigma H0 H). intros. elim (bool_is_true_or_false (ad_eq a1 a)); intros; rewrite H1. unfold state_correct_wrt_sign_with_offset in |- *.
	intros. simpl in H2. elim (bool_is_true_or_false (ad_eq a1 a3)); intros; rewrite H3 in H2. inversion H2. elim (H a a0). intros. elim (H0 a1 a2).
	intros. split with x. rewrite (ad_eq_complete _ _ H1) in H6. split.
	elim H4. intros. rewrite <- (ad_eq_complete _ _ H3). rewrite (ad_eq_complete _ _ H1). exact H7. elim H4. elim H6. intros.
	rewrite H9 in H7. inversion H7. rewrite H12 in H10. exact (union_pl_correct_wrt_sign_invar _ _ _ H8 H10). simpl in |- *. rewrite (ad_eq_correct a1). reflexivity. simpl in |- *. rewrite (ad_eq_correct a).
	reflexivity. inversion H2. elim (ad_sum (ad_xor a a1)); intro y. elim y.
	intros x y0. rewrite y0. unfold state_correct_wrt_sign_with_offset in |- *. intros.
	rewrite (MapPut1_semantics prec_list x a a1 a0 a2) in H2. elim (bool_is_true_or_false (ad_eq a a3)); intros. rewrite H3 in H2.
	inversion H2. elim (H a a0). intros. split with x0. rewrite (ad_eq_complete _ _ H3) in H4. rewrite H5 in H4. exact H4. simpl in |- *.
	rewrite (ad_eq_correct a). reflexivity. rewrite H3 in H2. elim (bool_is_true_or_false (ad_eq a1 a3)); intros. rewrite H4 in H2.
	inversion H2. elim (H0 a1 a2). intros. split with x0. rewrite (ad_eq_complete _ _ H4) in H5. rewrite H6 in H5. exact H5. simpl in |- *.
	rewrite (ad_eq_correct a1). reflexivity. rewrite H4 in H2. inversion H2. exact y0. rewrite (ad_eq_comm a1 a) in H1. rewrite (ad_xor_eq_true _ _ y) in H1. inversion H1. intros. simpl in |- *. induction  a as [| p]. exact (union_mpl_correct_wrt_sign_invar_0 _ _ _ _ _ H2 H1). induction  p as [p Hrecp| p Hrecp| ].
	clear Hrecp. replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list m (union_mpl_0 (ad_x p) a0 m0)) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x (xI p)) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x (xI p)) a0 pa sigma H2 H1). reflexivity.
	clear Hrecp. replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list (union_mpl_0 (ad_x p) a0 m) m0) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x (xO p)) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x (xO p)) a0 pa sigma H2 H1). reflexivity.
	replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list m (union_mpl_0 ad_z a0 m0)) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x 1) a0 pa sigma H2 H1). reflexivity. simple induction s1. intros. simpl in |- *. exact H1.
	intros. simpl in |- *. induction  a as [| p]. replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list (union_mpl_0 ad_z a0 m) m0) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 ad_z a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) ad_z a0 pa sigma H1
  ...
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x (xI p)) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x (xI p)) a0 pa sigma H1 H2). reflexivity. replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list (union_mpl_0 (ad_x p) a0 m) m0) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x (xO p)) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x (xO p)) a0 pa sigma H1 H2). reflexivity.
	replace
  (state_correct_wrt_sign_with_offset
     (M2 prec_list m (union_mpl_0 ad_z a0 m0)) sigma pa) with
  (state_correct_wrt_sign_with_offset
     (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0)) sigma pa). exact
  (union_mpl_correct_wrt_sign_invar_0 (M2 prec_list m m0) 
     (ad_x 1) a0 pa sigma H1 H2). reflexivity. intros. simpl in |- *. elim (state_correct_wrt_sign_with_offset_M2 _ _ _ _ H3). elim (state_correct_wrt_sign_with_offset_M2 _ _ _ _ H4). intros. cut
  ...
  (state_correct_wrt_sign_with_offset (union_mpl m0 m2) sigma (pre_ad_I pa)). intros. unfold state_correct_wrt_sign_with_offset in |- *. intros.
	induction  a as [| p0]. elim (H10 ad_z p). intros. split with x. simpl in H12.
	exact H12. simpl in H11. exact H11. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. elim (H9 (ad_x p0) p).
	intros. split with x. simpl in H12. exact H12. simpl in H11.
	exact H11. elim (H10 (ad_x p0) p). intros. split with x. simpl in H12. exact H12. simpl in H11. exact H11. elim (H9 ad_z p). intros.
...
Lemma union_0_correct_wrt_sign_invar :
 ...
 predta_correct_wrt_sign d sigma ->
 union_0 d a0 a1 = SOME state s -> state_correct_wrt_sign s sigma.
Proof.
...
Lemma insert_ostate_correct_wrt_sign_invar :
 ...
 predta_correct_wrt_sign d sigma ->
 state_correct_wrt_sign s sigma ->
 predta_correct_wrt_sign (insert_ostate d a (SOME state s)) sigma.
Proof.
	unfold predta_correct_wrt_sign in |- *. unfold insert_ostate in |- *. intros.
	rewrite (MapPut_semantics state d a s) in H1. elim (bool_is_true_or_false (ad_eq a a0)); intros; rewrite H2 in H1.
...
Lemma udta_conv_0_ref_ok_invar_0 :
 ...
 MapGet state (udta_conv_0 d) (uad_conv_0 a) = SOME state s ->
...
Lemma udta_conv_1_ref_ok_invar_0 :
 forall (d : preDTA) (a : ad) (s : state) (c : ad) (p : prec_list) (b : ad),
 MapGet state (udta_conv_1 d) (uad_conv_1 a) = SOME state s ->
 MapGet prec_list s c = SOME prec_list p ->
 prec_occur p b ->
 exists s' : state,
   (exists p' : prec_list,
...
Proof.
	...
	intros. elim (bool_is_true_or_false (ad_eq a1 a)); intros; rewrite H2 in H1.
	simpl in H1. elim (bool_is_true_or_false (ad_eq a1 a2)); intros; rewrite H3 in H1. inversion H1. apply (union_pl_ref_ok_invar p a0 d). apply (H a1).
	simpl in |- *. rewrite (ad_eq_correct a1). reflexivity. apply (H0 a). simpl in |- *.
	rewrite (ad_eq_correct a). reflexivity. inversion H1. elim (ad_sum (ad_xor a a1)); intros y. elim y. intros x y0. rewrite y0 in H1. rewrite (MapPut1_semantics prec_list x a a1 a0 p) in H1. elim (bool_is_true_or_false (ad_eq a a2)); intros; rewrite H3 in H1. inversion H1. rewrite <- H5.
	apply (H0 a). simpl in |- *. rewrite (ad_eq_correct a). reflexivity. elim (bool_is_true_or_false (ad_eq a1 a2)); intros; rewrite H4 in H1;
  inversion H1. rewrite <- H6. apply (H a1). simpl in |- *. rewrite (ad_eq_correct a1). reflexivity. exact y0. rewrite (ad_eq_comm a1 a) in H2. rewrite (ad_xor_eq_true _ _ y) in H2. inversion H2. intros. simpl in |- *. unfold state_ref_ok in |- *. intros. cut (state_ref_ok m0 d). cut (state_ref_ok m d).
	intros. induction  a as [| p1]. induction  a0 as [| p1]. simpl in H3. exact (H _ _ _ H1 H4 ad_z p0 H3). induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]; simpl in H3. exact (H5 _ _ H3). exact (H _ _ _ H1 H4 _ _ H3). exact (H5 _ _ H3). induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]. clear Hrecp1.
	induction  a0 as [| p2]. simpl in H3. exact (H4 _ _ H3). cut (state_ref_ok (M1 prec_list (ad_x p1) p) d). intro. induction  p2 as [p2 Hrecp2| p2 Hrecp2| ]; simpl in H3. exact (H0 _ _ _ H6 H5 _ _ H3). exact (H4 _ _ H3). exact (H0 _ _ _ H6 H5 _ _ H3).
	unfold state_ref_ok in |- *. intros. apply (H1 (ad_double_plus_un a) p3).
	induction  a as [| p4]; simpl in |- *; exact H6. cut (state_ref_ok (M1 prec_list (ad_x p1) p) d). intro. induction  a0 as [| p2]. simpl in H3. exact (H _ _ _ H6 H4 _ _ H3).
	induction  p2 as [p2 Hrecp2| p2 Hrecp2| ]; simpl in H3. exact (H5 _ _ H3). exact (H _ _ _ H6 H4 _ _ H3). exact (H5 _ _ H3). unfold state_ref_ok in |- *. intros. apply (H1 (ad_double a) p2). induction  a as [| p3]; simpl in |- *; exact H6. cut (state_ref_ok (M1 prec_list ad_z p) d). intros. induction  a0 as [| p1]. simpl in H3. exact (H4 _ _ H3). induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]; simpl in H3. exact (H0 _ _ _ H6 H5 _ _ H3). exact (H4 _ _ H3). exact (H0 _ _ _ H6 H5 _ _ H3). unfold state_ref_ok in |- *. intros. apply (H1 (ad_double_plus_un a) p1). induction  a as [| p2]; exact H6. unfold state_ref_ok in |- *. intros. apply (H2 (ad_double a1) p1).
	induction  a1 as [| p2]; exact H4. unfold state_ref_ok in |- *. intros. apply (H2 (ad_double_plus_un a1) p1). induction  a1 as [| p2]; exact H4.
Qed.
...
Proof.
  ...
  (MapPut_semantics state (u_merge p p0) (new_preDTA_ad (u_merge p p0))
     (union_mpl x x0)) in H1. elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) a1));
  intros; rewrite H4 in H1. inversion H1.
	elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) b));
  ...
  . rewrite
  (MapPut_semantics state (u_merge p p0) (new_preDTA_ad (u_merge p p0)) x)
   in H1. elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) a1));
  intros; rewrite H4 in H1.
	inversion H1. rewrite <- H6. elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) b));
  intros; rewrite H5.
	split with x. reflexivity. elim (preDTA_ref_ok_def (u_merge p p0)).
	intros. rewrite <- H6 in H2. exact (H7 (u_merge_ref_ok_invar _ _ H H0) (uad_conv_0 a) x y0 c pl H2 b H3). elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) b));
  ...
  . rewrite
  (MapPut_semantics state (u_merge p p0) (new_preDTA_ad (u_merge p p0)) x)
   in H1. elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) b));
  intros; rewrite H4.
	split with x. reflexivity. elim (bool_is_true_or_false (ad_eq (new_preDTA_ad (u_merge p p0)) a2));
...
Proof.
  ...
  (MapPut_semantics state (u_merge p p0)
     (ad_min
        ...
        (ad_double_plus_un (new_preDTA_ad (udta_conv_1_aux p0))))
