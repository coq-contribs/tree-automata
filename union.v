...
Proof.
	simple induction a. split with ad_z. left. simpl in |- *. trivial. simple induction p.
	intros. split with (ad_x p0). right. simpl in |- *. trivial. intros.
	split with (ad_x p0). left. simpl in |- *. trivial. split with ad_z.
...
Lemma u_conv_0_invar_0 :
 ...
 MapGet state (udta_conv_0 d) (uad_conv_0 a) = SOME state (umpl_conv_0 ladj).
Proof.
	...
	cut (ad_eq_1 p p = true). intro. rewrite H1 in H. rewrite H1.
	inversion H. trivial. exact (aux_ad_eq_1_0 p). intros. 
	cut (ad_eq a a1 = true). intro. exact (ad_eq_complete a a1 H0).
	cut (ad_eq a a1 = true \/ ad_eq a a1 = false). intros. elim H0.
	intros. assumption. intro. 
	cut (MapGet state (M1 state a a0) a1 = NONE state).
	intro. rewrite H2 in H. inversion H.
	exact (M1_semantics_2 state a a1 a0 H1).
	exact (bool_is_true_or_false (ad_eq a a1)).
	intro. intro. intro. intro. simple induction a. exact (H ad_z).
	simple induction p. intros. simpl in |- *. simpl in H2. 
	exact (H0 (ad_x p0) ladj H2). intros. simpl in |- *. simpl in H2.
	exact (H (ad_x p0) ladj H2). intros. simpl in H1. simpl in |- *.
	exact (H0 ad_z ladj H1).
Qed.

Lemma u_conv_0_invar_1 :
 forall (s : state) (c : ad) (p : prec_list),
 MapGet prec_list s c = SOME prec_list p ->
 MapGet prec_list (umpl_conv_0 s) c = SOME prec_list (upl_conv_0 p).
Proof.
	simple induction s. intros. simpl in H. inversion H. intros.
	simpl in |- *. simpl in H. cut (ad_eq a c = true). intro.
	rewrite H0 in H. rewrite H0. inversion H. trivial.
	cut (ad_eq a c = true \/ ad_eq a c = false). intro. 
	elim H0; intros. assumption. rewrite H1 in H.
	inversion H. exact (bool_is_true_or_false (ad_eq a c)).
	intro. intro. intro. intro. simple induction c. simpl in |- *. intros.
	exact (H ad_z p H1). simple induction p. intros. simpl in H2.
	simpl in |- *. exact (H0 (ad_x p0) p1 H2). intros. simpl in |- *.
	simpl in H2. exact (H (ad_x p0) p1 H2).
	intros. simpl in |- *. simpl in H1. exact (H0 ad_z p0 H1).
Qed.
...
Proof.
	...
	cut (ad_eq_1 p p0 = true \/ ad_eq_1 p p0 = false). intro. elim H0; intros.
	...
	inversion H. exact (bool_is_true_or_false (ad_eq_1 p p0)).
	inversion H. inversion H. simpl in H. simpl in |- *.
	cut (ad_eq_1 (xO p) p0 = true \/ ad_eq_1 (xO p) p0 = false). intro.
	...
	exact (bool_is_true_or_false (ad_eq_1 (xO p) p0)). simpl in |- *. simpl in H. 
	cut (ad_eq_1 1 p0 = true \/ ad_eq_1 1 p0 = false). intro. elim H0; intros.
	...
	exact (bool_is_true_or_false (ad_eq_1 1 p0)). intros. induction  a as [| p].
	simpl in H1. unfold udta_conv_0 in H. simpl in |- *. apply (H ad_z ladj).
	simpl in |- *. trivial. induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. unfold udta_conv_0 in H0.
	apply (H0 (ad_x p) ladj). simpl in |- *. simpl in H1. trivial. simpl in |- *. simpl in H1.
	unfold udta_conv_0 in H. exact (H (ad_x p) ladj H1). simpl in |- *. simpl in H1.
	unfold udta_conv_0 in H0. exact (H0 ad_z ladj H1).
Qed.
...
Proof.
	...
	cut (ad_eq_1 p p1 = true \/ ad_eq_1 p p1 = false). intros. elim H1; intros.
	...
	rewrite H2 in H0. inversion H0. exact (bool_is_true_or_false (ad_eq_1 p p1)).
	intros. inversion H0. intros. inversion H. simple induction p0. intros. inversion H0.
	intros. simpl in H0. simpl in |- *. cut (ad_eq_1 p p1 = true \/ ad_eq_1 p p1 = false).
	...
	exact (bool_is_true_or_false (ad_eq_1 p p1)). intros. inversion H.
	...
	simpl in |- *. intros. exact (H ad_z p H1). simple induction p. intros. simpl in |- *. simpl in H2.
	exact (H0 (ad_x p0) p1 H2). intros; simpl in |- *. simpl in H2. 
	exact (H (ad_x p0) p1 H2). intros; simpl in |- *. simpl in H1. exact (H0 ad_z p0 H1).
Qed.

Lemma u_conv_0_invar_4 :
 forall (d : preDTA) (a : ad) (ladj : state),
 MapGet state (udta_conv_0 d) (uad_conv_0 a) = SOME state ladj ->
...
Proof.
	...
	simpl in |- *. intros. cut (ad_eq_1 p p0 = true \/ ad_eq_1 p p0 = false). intro.
	elim H0; intros. rewrite H1 in H. inversion H. split with a0. trivial.
	rewrite H1 in H. inversion H. exact (bool_is_true_or_false (ad_eq_1 p p0)).
	intro. intro. intro. intro. simple induction a. simpl in |- *. intros. exact (H ad_z ladj H1).
	simple induction p. intros. simpl in H2. exact (H0 (ad_x p0) ladj H2). intros.
	simpl in H2. exact (H (ad_x p0) ladj H2). simpl in |- *. intros. exact (H0 ad_z ladj H1).
Qed.
...
Lemma u_conv_0_invar_5 :
 ...
 MapGet state (udta_conv_0 d) (uad_conv_0 a) = SOME state ladj ->
 exists ladj0 : _,
   ladj = umpl_conv_0 ladj0 /\ MapGet state d a = SOME state ladj0.
Proof.
...
Lemma u_conv_0_invar_6 :
 forall (s : state) (c : ad) (p : prec_list),
 MapGet prec_list (umpl_conv_0 s) c = SOME prec_list p ->
...
Proof.
	...
	cut (ad_eq_1 p0 p2 = true \/ ad_eq_1 p0 p2 = false). intro. elim H2. intros. 
	rewrite H3 in H1. inversion H1. split with a0. trivial. intro. rewrite H3 in H1.
	inversion H1. exact (bool_is_true_or_false (ad_eq_1 p0 p2)). intros. inversion H1.
	...
	cut (ad_eq_1 p0 p2 = true \/ ad_eq_1 p0 p2 = false). intro. elim H2; intro. 
	rewrite H3 in H1. inversion H1. split with a0. trivial. rewrite H3 in H1. inversion H1.
	exact (bool_is_true_or_false (ad_eq_1 p0 p2)). intros. inversion H0. intro.
	...
	intro. intro. simple induction c. simpl in |- *. intros. simpl in H1. exact (H ad_z p H1). simple induction p.
	intros. simpl in H2. exact (H0 (ad_x p0) p1 H2). intros. simpl in H2. 
	exact (H (ad_x p0) p1 H2). simpl in |- *. intros. exact (H0 ad_z p0 H1).
Qed.
...
Lemma u_conv_0_invar_7 :
 ...
 MapGet prec_list (umpl_conv_0 s) c = SOME prec_list p ->
 exists p0 : prec_list,
   p = upl_conv_0 p0 /\ MapGet prec_list s c = SOME prec_list p0.
Proof.
...
Lemma u_conv_0_invar_8 :
 ...
 MapGet state (udta_conv_0 p0) a0 = SOME state s0 ->
...
Proof.
	...
	induction  a as [| p]. split with ad_z. unfold uad_conv_0 in |- *. induction  a1 as [| p].
	trivial. simpl in H. induction  p as [p Hrecp| p Hrecp| ]; inversion H.
	split with (ad_x p). simpl in |- *. induction  a1 as [| p1]. simpl in H.
	inversion H. induction  p1 as [p1 Hrecp1| p1 Hrecp1| ]. simpl in H. inversion H.
	simpl in H. cut (ad_eq_1 p p1 = true). intro. cut (p1 = p).
	intro. rewrite H1. trivial. symmetry  in |- *. 
	exact (aux_ad_eq_1_1 p p1 H0).
	cut (ad_eq_1 p p1 = true \/ ad_eq_1 p p1 = false). intro.
	elim H0; intros. assumption. rewrite H1 in H. inversion H.
	exact (bool_is_true_or_false (ad_eq_1 p p1)).
 	simpl in H. inversion H. unfold udta_conv_0 in |- *. intro. intro.
	intro. intro. simple induction a0. intros. split with ad_z. simpl in |- *.
	trivial. simple induction p. intros. inversion H2. intros.
	split with (ad_x p1). simpl in |- *. trivial. intros. inversion H1.
Qed.
...
Proof.
  ...
  (MapGet state (udta_conv_0 d) (uad_conv_0 a) =
   SOME state (umpl_conv_0 ladj)). intros.
...
Proof.
	...
	cut (MapGet prec_list (umpl_conv_0 s) c = SOME prec_list (upl_conv_0 l)). intros.
...
Lemma u_conv_1_invar_0 :
 forall (d : preDTA) (a : ad) (ladj : state),
 MapGet state d a = SOME state ladj ->
 MapGet state (udta_conv_1 d) (uad_conv_1 a) = SOME state (umpl_conv_1 ladj).
Proof.
	simple induction d. intros. simpl in H. inversion H. intros. 
	...
	simpl in |- *. simpl in H. cut (ad_eq_1 p p = true). intro. rewrite H1. simpl in H.
	rewrite H1 in H. inversion H. trivial. exact (aux_ad_eq_1_0 p).
 	cut (ad_eq a a1 = true \/ ad_eq a a1 = false). intros. elim H0. intros.
	exact (ad_eq_complete a a1 H1). intro. 
	cut (MapGet state (M1 state a a0) a1 = NONE state).
	intro. rewrite H2 in H. inversion H.
	exact (M1_semantics_2 state a a1 a0 H1).
	exact (bool_is_true_or_false (ad_eq a a1)).
	intro. intro. intro. intro. simple induction a. simpl in |- *. intros. 
	exact (H ad_z ladj H1). simple induction p. intros. simpl in |- *. simpl in H2.
	exact (H0 (ad_x p0) ladj H2). intros. simpl in |- *. simpl in H2.
	exact (H (ad_x p0) ladj H2). intros. simpl in |- *. simpl in H1.
	exact (H0 ad_z ladj H1).
Qed.
...
Lemma u_conv_1_invar_1 :
 forall (s : state) (c : ad) (p : prec_list),
 MapGet prec_list s c = SOME prec_list p ->
 MapGet prec_list (umpl_conv_1 s) c = SOME prec_list (upl_conv_1 p).
Proof.
	simple induction s. simpl in |- *. intros. inversion H.
	intros. simpl in H. simpl in |- *. cut (ad_eq a c = true). intro.
	rewrite H0 in H. rewrite H0. inversion H. trivial.
	cut (ad_eq a c = true \/ ad_eq a c = false). intros.
	elim H0; intros. assumption. rewrite H1 in H. inversion H.
	exact (bool_is_true_or_false (ad_eq a c)).
	intro. intro. intro. intro. simple induction c. simpl in |- *. intros.
	exact (H ad_z p H1). simple induction p. intros. simpl in H2.
	simpl in |- *. exact (H0 (ad_x p0) p1 H2). intros. simpl in |- *. 
	simpl in H2. exact (H (ad_x p0) p1 H2).
...
Proof.
	...
	cut (ad_eq_1 p0 p2 = true \/ ad_eq_1 p0 p2 = false). intros. elim H2; intros.
	...
	exact (bool_is_true_or_false (ad_eq_1 p0 p2)). intros. simpl in H1. 
	...
	cut (ad_eq_1 p0 p1 = true \/ ad_eq_1 p0 p1 = false). intro. elim H1; intros.
	...
	inversion H0. exact (bool_is_true_or_false (ad_eq_1 p0 p1)).
	...
	simple induction a. simpl in |- *. intros. exact (H ad_z ladj H1). simple induction p. intros.
	simpl in |- *. simpl in H2. exact (H0 (ad_x p0) ladj H2). intros. simpl in |- *. simpl in H2.
	exact (H (ad_x p0) ladj H2). intros. simpl in |- *. simpl in H1. 
	exact (H0 ad_z ladj H1).
Qed.
...
Proof.
	...
	simpl in |- *. simpl in H0. cut (ad_eq_1 p p1 = true \/ ad_eq_1 p p1 = false). intro.
	...
	rewrite H2 in H0. inversion H0. exact (bool_is_true_or_false (ad_eq_1 p p1)).
	intros. inversion H0. intros. inversion H. simple induction p0. intros. inversion H0.
	intros. simpl in |- *. simpl in H0. cut (ad_eq_1 p p1 = true \/ ad_eq_1 p p1 = false).
	...
	inversion H0. exact (bool_is_true_or_false (ad_eq_1 p p1)). intros. inversion H.
	...
	intro. simple induction c. simpl in |- *. intros. exact (H ad_z p H1). simple induction p. intros.
	simpl in |- *. simpl in H2. exact (H0 (ad_x p0) p1 H2). intros. simpl in |- *. simpl in H2.
	exact (H (ad_x p0) p1 H2). intros. simpl in |- *. simpl in H1. exact (H0 ad_z p0 H1).
Qed.

Lemma u_conv_1_invar_4 :
 forall (d : preDTA) (a : ad) (ladj : state),
 MapGet state (udta_conv_1 d) (uad_conv_1 a) = SOME state ladj ->
...
Proof.
	...
	simpl in H0. elim (bool_is_true_or_false (ad_eq_1 p p1)); intro; rewrite H1 in H0.
	...
	elim (bool_is_true_or_false (ad_eq_1 p p1)); intros; rewrite H1 in H0. inversion H0.
	...
	intro. intro. intro. intro. simple induction a. intros. simpl in H1. exact (H ad_z ladj H1).
	simple induction p. intros. simpl in H2. exact (H0 (ad_x p0) ladj H2). intros. simpl in H2.
	exact (H (ad_x p0) ladj H2). intros. simpl in H1. exact (H0 ad_z ladj H1).
Qed.
...
Lemma u_conv_1_invar_5 :
 ...
 MapGet state (udta_conv_1 d) (uad_conv_1 a) = SOME state ladj ->
 exists ladj0 : _,
   ladj = umpl_conv_1 ladj0 /\ MapGet state d a = SOME state ladj0.
Proof.
...
Lemma u_conv_1_invar_6 :
 forall (s : state) (c : ad) (p : prec_list),
 MapGet prec_list (umpl_conv_1 s) c = SOME prec_list p ->
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq_1 p p1)); intro; rewrite H1 in H0.
	...
	simpl in |- *. intros. exact (H ad_z p H1). simple induction p; intros. simpl in H2.
	exact (H0 (ad_x p0) p1 H2). simpl in H2. exact (H (ad_x p0) p1 H2).
	simpl in H1. exact (H0 ad_z p0 H1).
Qed.
...
Lemma u_conv_1_invar_7 :
 ...
 MapGet prec_list (umpl_conv_1 s) c = SOME prec_list p ->
 exists p0 : prec_list,
   p = upl_conv_1 p0 /\ MapGet prec_list s c = SOME prec_list p0.
Proof.
...
Lemma u_conv_1_invar_8 :
 forall (p0 : preDTA) (a0 : ad) (s0 : state),
 MapGet state (udta_conv_1 p0) a0 = SOME state s0 ->
...
Proof.
	...
	split with (ad_x p1). simpl in |- *. trivial. intros. simpl in H0.
	inversion H0. intros. simpl in H. split with ad_z. simpl in |- *.
	trivial.  intro. intro. intro. intro. simple induction a0.
	simpl in |- *. intros. inversion H1. simple induction p. intros.
	split with (ad_x p1). simpl in |- *. trivial. intros. simpl in H2.
	inversion H2. intros. split with ad_z. simpl in |- *. trivial.
Qed.
...
Proof.
  ...
  (MapGet state (udta_conv_1 d) (uad_conv_1 a) =
   SOME state (umpl_conv_1 ladj)). intros.
...
Proof.
	...
	cut (MapGet prec_list (umpl_conv_1 s) c = SOME prec_list (upl_conv_1 l)). intros.
...
Lemma u_conv_disj :
 ...
 MapGet state (udta_conv_0 p0) a0 = SOME state s0 ->
 MapGet state (udta_conv_1 p1) a1 = SOME state s1 -> a0 <> a1.
Proof.
...
Lemma u_merge_0 :
 ...
 MapGet state (udta_conv_0 p0) a = SOME state s ->
 MapGet state (u_merge p0 p1) a = SOME state s.
Proof.
  ...
  (MapGet state (udta_conv_1 p1) a = NONE state \/
   (exists y : state, MapGet state (udta_conv_1 p1) a = SOME state y)).
	...
	elim (MapGet state (udta_conv_1 p1) a). left. trivial. right. split with a0.
	trivial. exact (H0 a).
...
Lemma u_merge_1 :
 forall (p0 p1 : preDTA) (a : ad) (s : state),
 MapGet state (udta_conv_1 p1) a = SOME state s ->
 MapGet state (u_merge p0 p1) a = SOME state s.
Proof.
	intros. unfold u_merge in |- *. cut
  (eqm state
...
Lemma u_merge_0r :
 ...
 MapGet state (u_merge p0 p1) a = SOME state s ->
 forall b : ad,
 a = uad_conv_0 b -> MapGet state (udta_conv_0 p0) a = SOME state s.
Proof.
	intros. cut
  (eqm state
     (MapGet state (MapMerge state (udta_conv_0 p0) (udta_conv_1 p1)))
  ...
  (MapGet state (MapMerge state (udta_conv_0 p0) (udta_conv_1 p1))
   ...
   | SOME y' => SOME state y'
	...
	cut
  (MapGet state (udta_conv_1 p1) (uad_conv_0 b) = NONE state \/
   (exists s : state,
      MapGet state (udta_conv_1 p1) (uad_conv_0 b) = SOME state s)).
	...
	simple induction o. left. trivial. intro. right. split with a0. trivial.
...
Lemma u_merge_1r :
 ...
 MapGet state (u_merge p0 p1) a = SOME state s ->
 forall b : ad,
 a = uad_conv_1 b -> MapGet state (udta_conv_1 p1) a = SOME state s.
Proof.
	intros. cut
  (eqm state
     (MapGet state (MapMerge state (udta_conv_0 p0) (udta_conv_1 p1)))
  ...
  (MapGet state (MapMerge state (udta_conv_0 p0) (udta_conv_1 p1))
   ...
   | SOME y' => SOME state y'
	...
	cut
  (MapGet state (udta_conv_1 p1) (uad_conv_1 b) = NONE state \/
   (exists s : _, MapGet state (udta_conv_1 p1) (uad_conv_1 b) = SOME state s)). intro.
	...
	generalize (MapGet state (udta_conv_1 p1) (uad_conv_1 b)). simple induction o.
...
Proof.
  ...
  (exists ladj0 : state,
     ladj = umpl_conv_0 ladj0 /\ MapGet state p0 a0 = SOME state ladj0).
...
Proof.
  ...
  (exists ladj0 : state,
     ladj = umpl_conv_1 ladj0 /\ MapGet state p1 a0 = SOME state ladj0).
	intros. elim H2. intros. elim H3. intros. exact (H p0 p1 H0 x H4).
...
Fixpoint union_mpl_0 (c : ad) (pl : prec_list) (s : state) {struct s} :
 state :=
  ...
  | M2 s0 s1 =>
      match c with
      | ad_z => M2 prec_list (union_mpl_0 ad_z pl s0) s1
      | ad_x p =>
          match p with
          | xH => M2 prec_list s0 (union_mpl_0 ad_z pl s1)
          | xO p' => M2 prec_list (union_mpl_0 (ad_x p') pl s0) s1
          | xI p' => M2 prec_list s0 (union_mpl_0 (ad_x p') pl s1)
          end
      end
  end.
...
Definition mpl_compat_7_def (s : state) : Prop :=
  ...
  MapGet prec_list (union_mpl_0 c pl s) c = SOME prec_list (union_pl pl l).
...
Proof.
	unfold mpl_compat_7_def in |- *. intros. simpl in H. elim (bool_is_true_or_false (ad_eq a c)); intros; rewrite H0 in H;
  inversion H. rewrite (ad_eq_complete a c H0). simpl in |- *.
	rewrite (ad_eq_correct c). simpl in |- *. rewrite (ad_eq_correct c). trivial.
Qed.
...
Proof.
	unfold mpl_compat_7_def in |- *. intros. induction  c as [| p]. simpl in |- *. apply (H ad_z pl l).
	simpl in H1. assumption. induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. apply (H0 (ad_x p) pl l).
	simpl in H1. assumption. simpl in |- *. apply (H (ad_x p) pl l). simpl in H1. assumption.
	simpl in H1. simpl in |- *. exact (H0 ad_z pl l H1).
Qed.
...
Definition mpl_compat_8_def (s : state) : Prop :=
  ...
  MapGet prec_list s c = SOME prec_list l ->
  a <> c -> MapGet prec_list (union_mpl_0 a pl s) c = SOME prec_list l.
...
Proof.
	unfold mpl_compat_8_def in |- *. intros. simpl in H. elim (bool_is_true_or_false (ad_eq a c)); intros; rewrite H1 in H. inversion H. simpl in |- *. elim (bool_is_true_or_false (ad_eq a1 a)); intro; rewrite H2. simpl in |- *. elim (bool_is_true_or_false (ad_eq a1 c)). intro.
	elim (H0 (ad_eq_complete a1 c H4)). intro. rewrite (ad_eq_complete a1 a H2) in H4.
	rewrite H1 in H4. inversion H4. elim (ad_sum (ad_xor a a1)). intro y. elim y.
	intros x y0. rewrite y0. rewrite (MapPut1_semantics prec_list x a a1 l pl y0 c).
	rewrite H1. trivial. intro y. rewrite (ad_eq_comm a1 a) in H2.
	rewrite (ad_xor_eq_true a a1 y) in H2. inversion H2. inversion H.
Qed.
...
Proof.
	unfold mpl_compat_8_def in |- *. intros. induction  a as [| p]; [ induction  c as [| p] | induction  c as [| p0] ]. elim (H2 (refl_equal ad_z)).
	simpl in |- *. induction  p as [p Hrecp| p Hrecp| ]. simpl in H1. assumption. simpl in H1. apply (H ad_z (ad_x p) pl l H1).
	intro. inversion H3. simpl in H1. assumption. simpl in |- *. induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. simpl in H1.
	assumption. simpl in |- *. apply (H (ad_x p) ad_z pl l H1). intro. inversion H3. simpl in |- *.
	simpl in H1. assumption. simpl in |- *. induction  p as [p Hrecp| p Hrecp| ]. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in |- *. simpl in H1.
	apply (H0 (ad_x p) (ad_x p0) pl l H1). intro. inversion H3. elim H2. trivial. rewrite H5.
	trivial. simpl in |- *. simpl in H1. assumption. simpl in |- *. apply (H0 (ad_x p) ad_z pl l H1).
	intro. inversion H3. simpl in |- *. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in H1. assumption.
	apply (H (ad_x p) (ad_x p0) pl l H1). intro. inversion H3. rewrite H5 in H2. elim H2.
	trivial. simpl in H1. assumption. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in |- *. simpl in H1. simpl in |- *.
	apply (H0 ad_z (ad_x p0) pl l). assumption. intro. inversion H3. simpl in |- *. simpl in H1.
...
Proof.
	unfold mpl_compat in |- *. intros. inversion H0. simpl in H5. rewrite (ad_eq_correct c) in H5.
	inversion H5. apply (rec_st d (union_mpl_0 c l (M0 prec_list)) c tl l). simpl in |- *.
	rewrite (ad_eq_correct c). trivial. assumption.
Qed.
...
Proof.
	...
	rewrite (ad_eq_correct c) in H5. inversion H5. 
	apply
  (rec_st d (union_mpl_0 c l (M1 prec_list c pl0)) c tl (union_pl l pl0)).
	simpl in |- *. rewrite (ad_eq_correct c). simpl in |- *. rewrite (ad_eq_correct c). trivial.
	apply (union_pl_0d d l pl0 tl). apply (H c l pl0). simpl in |- *. rewrite (ad_eq_correct c).
	trivial. simpl in |- *. rewrite (ad_eq_correct c). trivial. assumption.
Qed.
...
Proof.
	intros. unfold mpl_compat in H. inversion H1. apply (rec_st d (union_mpl_0 c pl (M1 prec_list c0 pl0)) c tl l). simpl in |- *. elim (bool_is_true_or_false (ad_eq c c0)); intros; rewrite H8. elim (H0 (ad_eq_complete c c0 H8)). elim (ad_sum (ad_xor c0 c)).
	intro y. elim y; intros x y0. rewrite y0. cut
  (MapGet prec_list (MapPut1 prec_list c0 pl0 c pl x) c =
   (fun a0 : ad =>
    match ad_eq c0 a0 with
    | true => SOME prec_list pl0
    | false =>
        match ad_eq c a0 with
        | true => SOME prec_list pl
        | false => NONE prec_list
        end
    end) c). intro. rewrite <- (ad_eq_comm c c0) in H9.
	rewrite H8 in H9. rewrite (ad_eq_correct c) in H9. rewrite H9. simpl in H6.
	rewrite (ad_eq_correct c) in H6. inversion H6. trivial. exact (MapPut1_semantics prec_list x c0 c pl0 pl y0 c). intro y. rewrite (ad_eq_comm c c0) in H8. rewrite (ad_xor_eq_true c0 c y) in H8. inversion H8. assumption.
Qed.

Lemma union_s0d_2_0 :
 forall (d : preDTA) (pl : prec_list) (s0 s1 : state) (tl : term_list),
 mpl_compat (M1 prec_list ad_z pl) (M2 prec_list s0 s1) ->
 state_reconnait d (M1 prec_list ad_z pl) (app ad_z tl) ->
 state_reconnait d (union_mpl_0 ad_z pl (M2 prec_list s0 s1)) (app ad_z tl).
Proof.
	intro. intro. simple induction s0. intros. simpl in |- *. inversion H0. apply (rec_st d (M2 prec_list (M1 prec_list ad_z pl) s1) ad_z tl l). simpl in |- *. simpl in H5. inversion H5. trivial.
	assumption. intros. unfold union_mpl_0 in |- *. elim (bool_is_true_or_false (ad_eq ad_z a)); intros; rewrite H1. apply
  (rec_st d (M2 prec_list (M1 prec_list ad_z (union_pl pl a0)) s1) ad_z tl
     (union_pl pl a0)). simpl in |- *. trivial. inversion H0. simpl in H6. inversion H6.
	apply (union_pl_0d d l a0 tl). apply (mpl_compat_0 ad_z l a0). rewrite <- (ad_eq_complete ad_z a H1) in H. rewrite H9 in H. apply (mpl_compat_sym (M1 prec_list ad_z a0) (M1 prec_list ad_z l)). apply (mpl_compat_3 (M1 prec_list ad_z a0) s1 l). apply
  (mpl_compat_sym (M1 prec_list ad_z l)
     (M2 prec_list (M1 prec_list ad_z a0) s1)). assumption. assumption. inversion H0.
	apply
  (rec_st d
     (M2 prec_list
        (MapMerge prec_list (M1 prec_list ad_z pl) (M1 prec_list a a0)) s1)
     ad_z tl l). cut
  (MapGet prec_list
     (MapMerge prec_list (M1 prec_list ad_z pl) (M1 prec_list a a0)) ad_z =
   (fun a1 : ad =>
    match MapGet prec_list (M1 prec_list a a0) a1 with
    | NONE => MapGet prec_list (M1 prec_list ad_z pl) a1
    | SOME y' => SOME prec_list y'
    end) ad_z). intro. cut
  (MapGet prec_list
     (M2 prec_list
        (MapMerge prec_list (M1 prec_list ad_z pl) (M1 prec_list a a0)) s1)
     ad_z =
   MapGet prec_list
     (MapMerge prec_list (M1 prec_list ad_z pl) (M1 prec_list a a0)) ad_z). intros.
	rewrite H9. rewrite H8. simpl in |- *. rewrite (ad_eq_comm ad_z a) in H1. rewrite H1. simpl in H6.
	inversion H6. trivial. simpl in |- *. trivial. exact
  (MapMerge_semantics prec_list (M1 prec_list ad_z pl) 
     (M1 prec_list a a0) ad_z). assumption. intros. simpl in |- *. simpl in H. cut
  (state_reconnait d (M2 prec_list (union_mpl_0 ad_z pl m) m0) (app ad_z tl)). intro. inversion H3. apply
  (rec_st d (M2 prec_list (M2 prec_list (union_mpl_0 ad_z pl m) m0) s1) ad_z
     tl l). simpl in |- *. assumption.
	assumption. apply (H m0 tl). apply (mpl_compat_sym (M2 prec_list m m0) (M1 prec_list ad_z pl)).
	apply (mpl_compat_3 (M2 prec_list m m0) s1 pl). exact
  (mpl_compat_sym (M1 prec_list ad_z pl)
     (M2 prec_list (M2 prec_list m m0) s1) H1). assumption.
Qed.

Lemma union_s0d_2_1 :
 forall (d : preDTA) (pl : prec_list) (s0 s1 : state) (tl : term_list),
 mpl_compat (M1 prec_list (ad_x 1) pl) (M2 prec_list s0 s1) ->
 state_reconnait d (M1 prec_list (ad_x 1) pl) (app (ad_x 1) tl) ->
 state_reconnait d (union_mpl_0 (ad_x 1) pl (M2 prec_list s0 s1))
   (app (ad_x 1) tl).
Proof.
	intros. cut (state_reconnait d (union_mpl_0 ad_z pl s1) (app ad_z tl)). simpl in |- *. intro.
	inversion H1. apply (rec_st d (M2 prec_list s0 (union_mpl_0 ad_z pl s1)) (ad_x 1) tl l).
	simpl in |- *. assumption. assumption. induction  s1 as [| a a0| s1_1 Hrecs1_1 s1_0 Hrecs1_0]. apply (union_s0d_0 d ad_z pl tl). apply (mpl_compat_sym (M0 prec_list) (M1 prec_list ad_z pl)). apply (mpl_compat_4 s0 (M0 prec_list) pl). 
	exact
  (mpl_compat_sym (M1 prec_list (ad_x 1) pl) (M2 prec_list s0 (M0 prec_list))
     H).
	inversion H0. simpl in H5. apply (rec_st d (M1 prec_list ad_z pl) ad_z tl pl). simpl in |- *.
	trivial. inversion H5. assumption. elim (classic (ad_z = a)). intro. rewrite <- H1. rewrite <- H1 in H. apply (union_s0d_1_0 d ad_z pl a0 tl). apply (mpl_compat_sym (M1 prec_list ad_z a0) (M1 prec_list ad_z pl)). apply (mpl_compat_4 s0 (M1 prec_list ad_z a0) pl). exact
  (mpl_compat_sym (M1 prec_list (ad_x 1) pl)
     (M2 prec_list s0 (M1 prec_list ad_z a0)) H). inversion H0.
	apply (rec_st d (M1 prec_list ad_z pl) ad_z tl pl). simpl in |- *. trivial. simpl in H6. inversion H6.
	assumption. intro. apply (union_s0d_1_1 d ad_z pl a a0 tl). apply (mpl_compat_sym (M1 prec_list a a0) (M1 prec_list ad_z pl)). apply (mpl_compat_4 s0 (M1 prec_list a a0) pl). exact
  (mpl_compat_sym (M1 prec_list (ad_x 1) pl)
     (M2 prec_list s0 (M1 prec_list a a0)) H). assumption. inversion H0.
	apply (rec_st d (M1 prec_list ad_z pl) ad_z tl pl). simpl in |- *. trivial. simpl in H6. inversion H6.
	assumption. simpl in |- *. cut (state_reconnait d (union_mpl_0 ad_z pl s1_1) (app ad_z tl)). intro.
	inversion H1. apply (rec_st d (M2 prec_list (union_mpl_0 ad_z pl s1_1) s1_0) ad_z tl l).
	simpl in |- *. assumption. assumption. apply Hrecs1_1. cut (mpl_compat (M1 prec_list ad_z pl) s1_1).
	intro. unfold mpl_compat in |- *. intros. unfold mpl_compat in H1. unfold MapGet in H2. elim (bool_is_true_or_false (ad_eq (ad_x 1) c)); intro; rewrite H4 in H2;
  inversion H2.
	apply (H1 ad_z p0 p1). simpl in |- *. rewrite H6. trivial. rewrite <- (ad_eq_complete (ad_x 1) c H4) in H3. simpl in H3. assumption. apply (mpl_compat_sym s1_1 (M1 prec_list ad_z pl)).
...
Proof.
	...
	induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) pl m0) (app (ad_x p) tl)). intro. inversion H3. apply
  (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p) pl m0)) (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption. apply (H0 d (ad_x p) pl tl).
	apply (mpl_compat_sym m0 (M1 prec_list (ad_x p) pl)). exact
  (mpl_compat_6 m m0 pl p
     (mpl_compat_sym (M1 prec_list (ad_x (xI p)) pl) (M2 prec_list m m0) H1)). inversion H2.
	simpl in H7. rewrite (aux_ad_eq_1_0 p) in H7. inversion H7. apply (rec_st d (M1 prec_list (ad_x p) l) (ad_x p) tl l). simpl in |- *. rewrite (aux_ad_eq_1_0 p). trivial. assumption.
	simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) pl m) (app (ad_x p) tl)). intro.
	inversion H3. apply
  (rec_st d (M2 prec_list (union_mpl_0 (ad_x p) pl m) m0) (ad_x (xO p)) tl l). simpl in |- *. assumption. assumption. apply (H d (ad_x p) pl tl). apply (mpl_compat_sym m (M1 prec_list (ad_x p) pl)). exact
  (mpl_compat_5 m m0 pl p
     (mpl_compat_sym (M1 prec_list (ad_x (xO p)) pl) (M2 prec_list m m0) H1)). inversion H2. apply (rec_st d (M1 prec_list (ad_x p) pl) (ad_x p) tl l). simpl in |- *. rewrite (aux_ad_eq_1_0 p). simpl in H7. rewrite (aux_ad_eq_1_0 p) in H7. inversion H7. trivial. assumption. 
...
Proof.
	intros. simpl in |- *. elim (bool_is_true_or_false (ad_eq a c)). intro. 
	elim (H0 (ad_eq_complete a c H2)). intro. rewrite H2. 
	elim (ad_sum (ad_xor c a)); intro y. elim y. intros x y0. rewrite y0. inversion H1.
	apply (rec_st d (MapPut1 prec_list c pl0 a pl x) c tl l).
	rewrite (MapPut1_semantics prec_list x c a pl0 pl y0 c). rewrite (ad_eq_correct c).
	simpl in H7. rewrite (ad_eq_correct c) in H7. inversion H7. trivial. trivial.
	rewrite (ad_xor_comm c a) in y. rewrite (ad_xor_eq_true a c y) in H2.
	inversion H2.
Qed.
...
Lemma union_s1d_1_1 :
 ...
 state_reconnait d (union_mpl_0 c pl (M1 prec_list c pl0)) (app c tl).
Proof.
	intros. simpl in |- *. rewrite (ad_eq_correct c). apply (rec_st d (M1 prec_list c (union_pl pl pl0)) c tl (union_pl pl pl0)). simpl in |- *. rewrite (ad_eq_correct c). trivial.
	inversion H0. simpl in H5. rewrite (ad_eq_correct c) in H5. inversion H5.
...
Proof.
	...
	exact (union_s1d_1_1 d pl a0 c tl H H0). intro. exact (union_s1d_1_0 d a1 pl a0 c tl H H2 H0). inversion H0. simpl in H5. elim (bool_is_true_or_false (ad_eq a c)).
	intro. exact (ad_eq_complete a c H7). intro. rewrite H7 in H5. inversion H5.
Qed.
...
Proof.
	unfold union_s_prd1 in |- *. intros. induction  c as [| p]. induction  a as [| p]. simpl in |- *. cut (state_reconnait d (union_mpl_0 ad_z pl m) (app ad_z tl)). intro. inversion H3. apply (rec_st d (M2 prec_list (union_mpl_0 ad_z pl m) m0) ad_z tl l). simpl in |- *. assumption. assumption.
	apply (H d ad_z pl ad_z tl). apply (mpl_compat_sym m (M1 prec_list ad_z pl)).
	apply (mpl_compat_3 m m0 pl). exact (mpl_compat_sym (M1 prec_list ad_z pl) (M2 prec_list m m0) H1). inversion H2. apply (rec_st d m ad_z tl l). simpl in H7.
	assumption. assumption. induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. inversion H2.
	apply (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p) pl m0)) ad_z tl l). simpl in |- *.
	simpl in H7. assumption. assumption. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) pl m) (app ad_z tl)). intro. inversion H3. apply (rec_st d (M2 prec_list (union_mpl_0 (ad_x p) pl m) m0) ad_z tl l). simpl in |- *. assumption. assumption. 
	apply (H d (ad_x p) pl ad_z tl). apply (mpl_compat_sym m (M1 prec_list (ad_x p) pl)).
	apply (mpl_compat_5 m m0 pl p). exact
  (mpl_compat_sym (M1 prec_list (ad_x (xO p)) pl) (M2 prec_list m m0) H1). inversion H2. simpl in H7. exact (rec_st d m ad_z tl l H7 H8).
	simpl in |- *. inversion H2. apply (rec_st d (M2 prec_list m (union_mpl_0 ad_z pl m0)) ad_z tl l).
	simpl in |- *. simpl in H7. assumption. assumption. induction  p as [p Hrecp| p Hrecp| ]. induction  a as [| p0]. simpl in |- *. inversion H2.
	apply
  (rec_st d (M2 prec_list (union_mpl_0 ad_z pl m) m0) (ad_x (xI p)) tl l). simpl in |- *.
	simpl in H7. assumption. assumption. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p0) pl m0) (app (ad_x p) tl)). intro. inversion H3. apply
  (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p0) pl m0)) (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption.
	apply (H0 d (ad_x p0) pl (ad_x p) tl). apply (mpl_compat_sym m0 (M1 prec_list (ad_x p0) pl)).
	apply (mpl_compat_6 m m0 pl p0). exact
  (mpl_compat_sym (M1 prec_list (ad_x (xI p0)) pl) (M2 prec_list m m0) H1). inversion H2. simpl in H7. exact (rec_st d m0 (ad_x p) tl l H7 H8).
	simpl in |- *. inversion H2. apply
  (rec_st d (M2 prec_list (union_mpl_0 (ad_x p0) pl m) m0) (ad_x (xI p)) tl l). simpl in |- *. simpl in H7. assumption. assumption. simpl in |- *.
	cut (state_reconnait d (union_mpl_0 ad_z pl m0) (app (ad_x p) tl)). intro. inversion H3.
	apply
  (rec_st d (M2 prec_list m (union_mpl_0 ad_z pl m0)) (ad_x (xI p)) tl l). simpl in |- *.
	assumption. assumption. apply (H0 d ad_z pl (ad_x p) tl). apply (mpl_compat_sym m0 (M1 prec_list ad_z pl)). apply (mpl_compat_4 m m0 pl). exact (mpl_compat_sym (M1 prec_list (ad_x 1) pl) (M2 prec_list m m0) H1). inversion H2. simpl in H7. exact (rec_st d m0 (ad_x p) tl l H7 H8). induction  a as [| p0]. simpl in |- *. cut (state_reconnait d (union_mpl_0 ad_z pl m) (app (ad_x p) tl)).
	intro. inversion H3. apply
  (rec_st d (M2 prec_list (union_mpl_0 ad_z pl m) m0) (ad_x (xO p)) tl l).
	simpl in |- *. assumption. assumption. apply (H d ad_z pl (ad_x p) tl). apply (mpl_compat_sym m (M1 prec_list ad_z pl)). apply (mpl_compat_3 m m0 pl). exact (mpl_compat_sym (M1 prec_list ad_z pl) (M2 prec_list m m0) H1). inversion H2. simpl in H7. exact (rec_st d m (ad_x p) tl l H7 H8).
	induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in |- *. inversion H2. apply
  (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p0) pl m0)) (ad_x (xO p)) tl l). simpl in |- *. simpl in H7. assumption. assumption. simpl in |- *.
	cut (state_reconnait d (union_mpl_0 (ad_x p0) pl m) (app (ad_x p) tl)). intro. inversion H3.
	apply
  (rec_st d (M2 prec_list (union_mpl_0 (ad_x p0) pl m) m0) (ad_x (xO p)) tl l). simpl in |- *.
	simpl in H8. assumption. assumption. apply (H d (ad_x p0) pl (ad_x p) tl). 
	apply (mpl_compat_sym m (M1 prec_list (ad_x p0) pl)). apply (mpl_compat_5 m m0 pl p0). 
	exact
  (mpl_compat_sym (M1 prec_list (ad_x (xO p0)) pl) (M2 prec_list m m0) H1). inversion H2. 
	simpl in H7. exact (rec_st d m (ad_x p) tl l H7 H8). simpl in |- *. inversion H2. simpl in H7.
	apply
  (rec_st d (M2 prec_list m (union_mpl_0 ad_z pl m0)) (ad_x (xO p)) tl l). simpl in |- *.
	assumption. assumption. induction  a as [| p]. simpl in |- *. inversion H2. simpl in H7.
	apply (rec_st d (M2 prec_list (union_mpl_0 ad_z pl m) m0) (ad_x 1) tl l). simpl in |- *. assumption.
	assumption. induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) pl m0) (app ad_z tl)).
	intro. inversion H3. apply (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p) pl m0)) (ad_x 1) tl l).
	simpl in |- *. simpl in H8. assumption. assumption. apply (H0 d (ad_x p) pl ad_z tl).
	apply (mpl_compat_sym m0 (M1 prec_list (ad_x p) pl)). apply (mpl_compat_6 m m0 pl p).
	exact
  (mpl_compat_sym (M1 prec_list (ad_x (xI p)) pl) (M2 prec_list m m0) H1). inversion H2.
	simpl in H7. exact (rec_st d m0 ad_z tl l H7 H8). simpl in |- *. inversion H2. simpl in H7.
	apply (rec_st d (M2 prec_list (union_mpl_0 (ad_x p) pl m) m0) (ad_x 1) tl l). simpl in |- *.
	assumption. assumption. simpl in |- *. cut (state_reconnait d (union_mpl_0 ad_z pl m0) (app ad_z tl)).
	intro. inversion H3. apply (rec_st d (M2 prec_list m (union_mpl_0 ad_z pl m0)) (ad_x 1) tl l).
	simpl in |- *. assumption. assumption. apply (H0 d ad_z pl ad_z tl). 
	apply (mpl_compat_sym m0 (M1 prec_list ad_z pl)). apply (mpl_compat_4 m m0 pl).
	exact (mpl_compat_sym (M1 prec_list (ad_x 1) pl) (M2 prec_list m m0) H1).
	inversion H2. simpl in H7. exact (rec_st d m0 ad_z tl l H7 H8).
Qed.
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a c)); intro. exact (ad_eq_complete a c H7).
  ...
  (union_s1d (M1 prec_list a a0) d a1 a2 c tl
	...
	inversion H0. simpl in H5. elim (bool_is_true_or_false (ad_eq a c)). intro.
	exact (ad_eq_complete a c H7). intro. rewrite H7 in H5. inversion H5. induction  s1 as [| a1 a2| s1_1 Hrecs1_1 s1_0 Hrecs1_0].
	...
	rewrite H1. rewrite H1 in H. rewrite H1 in H0. exact (union_s0d (M1 prec_list a1 a2) d c a0 tl H H0). inversion H0. simpl in H5. elim (bool_is_true_or_false (ad_eq a c)).
	intro. exact (ad_eq_complete a c H7). intro. rewrite H7 in H5. inversion H5.
...
Proof.
	...
	cut
  (state_reconnait d (M2 prec_list (union_mpl_0 ad_z a0 m) m0) (app ad_z tl)).
	intro. split. assumption.  assumption. cut (state_reconnait d (union_mpl_0 ad_z a0 m) (app ad_z tl)). intro. inversion H3. apply (rec_st d (M2 prec_list (union_mpl_0 ad_z a0 m) m0) ad_z tl l). simpl in |- *. assumption. assumption.
	cut
  (state_reconnait d (union_mpl m (M1 prec_list ad_z a0)) (app ad_z tl) /\
   state_reconnait d (union_mpl (M1 prec_list ad_z a0) m) (app ad_z tl)).
	intro. elim H3. intros. unfold union_mpl in H4. induction  m as [| a a1| m1 Hrecm1 m2 Hrecm0]. assumption.
	assumption. assumption. apply (H (M1 prec_list ad_z a0) d ad_z tl).
	exact (mpl_compat_3 m m0 a0 H1). inversion H2. simpl in H7.
	exact (rec_st d m ad_z tl l H7 H8). induction  p as [p Hrecp| p Hrecp| ]. cut
  (state_reconnait d (union_mpl_0 (ad_x (xI p)) a0 (M2 prec_list m m0))
     (app ad_z tl)). intro.
	split. assumption. assumption. simpl in |- *. inversion H2. simpl in H7.
	apply (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p) a0 m0)) ad_z tl l).
	simpl in |- *. assumption. assumption. cut
  (state_reconnait d (union_mpl_0 (ad_x (xO p)) a0 (M2 prec_list m m0))
     (app ad_z tl)). intro. split. assumption. assumption.
	simpl in |- *. cut
  (state_reconnait d (union_mpl m (M1 prec_list (ad_x p) a0)) (app ad_z tl) /\
   state_reconnait d (union_mpl (M1 prec_list (ad_x p) a0) m) (app ad_z tl)). intro. elim H3. intros. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) a0 m) (app ad_z tl)). intro. inversion H6.
	apply (rec_st d (M2 prec_list (union_mpl_0 (ad_x p) a0 m) m0) ad_z tl l).
	simpl in |- *. assumption. assumption. induction  m as [| a a1| m1 Hrecm1 m2 Hrecm0]. assumption. assumption.
	assumption. apply (H (M1 prec_list (ad_x p) a0) d ad_z tl).
	exact (mpl_compat_5 m m0 a0 p H1). inversion H2. simpl in H7. exact (rec_st d m ad_z tl l H7 H8). cut
  (state_reconnait d (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0))
     (app ad_z tl)). intros. split. assumption. assumption. inversion H2.
	apply (rec_st d (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0)) ad_z tl l).
	simpl in |- *. simpl in H7. assumption. assumption. induction  p as [p Hrecp| p Hrecp| ]. induction  a as [| p0].
	cut
  (state_reconnait d (union_mpl_0 ad_z a0 (M2 prec_list m m0))
     (app (ad_x (xI p)) tl)).
	intro. split. assumption. assumption. simpl in |- *. inversion H2.
	apply
  (rec_st d (M2 prec_list (union_mpl_0 ad_z a0 m) m0) (ad_x (xI p)) tl l).
	simpl in |- *. assumption. assumption. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. cut
  (state_reconnait d (union_mpl_0 (ad_x (xI p0)) a0 (M2 prec_list m m0))
     (app (ad_x (xI p)) tl)).
	intro. split. assumption. assumption. simpl in |- *. cut
  (state_reconnait d (union_mpl m0 (M1 prec_list (ad_x p0) a0))
     (app (ad_x p) tl) /\
   state_reconnait d (union_mpl (M1 prec_list (ad_x p0) a0) m0)
     (app (ad_x p) tl)). intros. elim H3. intros.
	cut (state_reconnait d (union_mpl_0 (ad_x p0) a0 m0) (app (ad_x p) tl)). intro.
	inversion H6. apply
  (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p0) a0 m0)) (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption. induction  m0 as [| a a1| m0_1 Hrecm0_1 m0_0 Hrecm0_0]; assumption.
	apply (H0 (M1 prec_list (ad_x p0) a0) d (ad_x p) tl). exact (mpl_compat_6 m m0 a0 p0 H1).
	inversion H2. simpl in H7. exact (rec_st d m0 (ad_x p) tl l H7 H8).
	cut
  (state_reconnait d (union_mpl_0 (ad_x (xO p0)) a0 (M2 prec_list m m0))
     (app (ad_x (xI p)) tl)). intro. split. assumption. assumption. simpl in |- *.
	inversion H2. apply
  (rec_st d (M2 prec_list (union_mpl_0 (ad_x p0) a0 m) m0) (ad_x (xI p)) tl l). simpl in |- *. simpl in H7. assumption. assumption.
	cut
  (state_reconnait d (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0))
     (app (ad_x (xI p)) tl)). intro. split; assumption. simpl in |- *.
	cut
  (state_reconnait d (union_mpl m0 (M1 prec_list ad_z a0)) (app (ad_x p) tl) /\
   state_reconnait d (union_mpl (M1 prec_list ad_z a0) m0) (app (ad_x p) tl)).
	intro. elim H3. intros. cut (state_reconnait d (union_mpl_0 ad_z a0 m0) (app (ad_x p) tl)). intro. inversion H6. apply
  (rec_st d (M2 prec_list m (union_mpl_0 ad_z a0 m0)) (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption.
	induction  m0 as [| a a1| m0_1 Hrecm0_1 m0_0 Hrecm0_0]; assumption. apply (H0 (M1 prec_list ad_z a0) d (ad_x p) tl).
	exact (mpl_compat_4 m m0 a0 H1). inversion H2. simpl in H7. exact (rec_st d m0 (ad_x p) tl l H7 H8). induction  a as [| p0]. cut
  (state_reconnait d (union_mpl_0 ad_z a0 (M2 prec_list m m0))
     (app (ad_x (xO p)) tl)). intro. split; assumption. simpl in |- *.
	cut (state_reconnait d (union_mpl_0 ad_z a0 m) (app (ad_x p) tl)). intro. 
	inversion H3. apply
  (rec_st d (M2 prec_list (union_mpl_0 ad_z a0 m) m0) (ad_x (xO p)) tl l). simpl in |- *. assumption. assumption. cut
  (state_reconnait d (union_mpl m (M1 prec_list ad_z a0)) (app (ad_x p) tl) /\
   state_reconnait d (union_mpl (M1 prec_list ad_z a0) m) (app (ad_x p) tl)). intro. elim H3. intros. induction  m as [| a a1| m1 Hrecm1 m2 Hrecm0]; assumption.
	apply (H (M1 prec_list ad_z a0) d (ad_x p) tl). exact (mpl_compat_3 m m0 a0 H1).
	inversion H2. simpl in H7. exact (rec_st d m (ad_x p) tl l H7 H8). induction  p0 as [p0 Hrecp0| p0 Hrecp0| ].
	cut
  (state_reconnait d (union_mpl_0 (ad_x (xI p0)) a0 (M2 prec_list m m0))
     (app (ad_x (xO p)) tl)). intro. split; assumption. simpl in |- *. inversion H2.
	apply
  (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p0) a0 m0)) (ad_x (xO p)) tl l).
	simpl in |- *. assumption. assumption. cut
  (state_reconnait d (union_mpl_0 (ad_x (xO p0)) a0 (M2 prec_list m m0))
     (app (ad_x (xO p)) tl)). intros. split; assumption. simpl in |- *.
	cut (state_reconnait d (union_mpl_0 (ad_x p0) a0 m) (app (ad_x p) tl)). intro.
	inversion H3. apply
  (rec_st d (M2 prec_list (union_mpl_0 (ad_x p0) a0 m) m0) (ad_x (xO p)) tl l). simpl in |- *. simpl in H8. assumption. assumption.
	cut
  (state_reconnait d (union_mpl m (M1 prec_list (ad_x p0) a0))
     (app (ad_x p) tl) /\
   state_reconnait d (union_mpl (M1 prec_list (ad_x p0) a0) m)
     (app (ad_x p) tl)).
	intro. elim H3. intros. induction  m as [| a a1| m1 Hrecm1 m2 Hrecm0]; assumption. apply (H (M1 prec_list (ad_x p0) a0) d (ad_x p) tl). exact (mpl_compat_5 m m0 a0 p0 H1). inversion H2. simpl in H7.
	exact (rec_st d m (ad_x p) tl l H7 H8). cut
  (state_reconnait d (union_mpl_0 (ad_x 1) a0 (M2 prec_list m m0))
     (app (ad_x (xO p)) tl)). intro. split; assumption. simpl in |- *.
	inversion H2. apply
  (rec_st d (M2 prec_list m (union_mpl_0 ad_z a0 m0)) (ad_x (xO p)) tl l). simpl in H7. simpl in |- *. assumption. assumption. induction  a as [| p]. cut
  (state_reconnait d (union_mpl_0 ad_z a0 (M2 prec_list m m0))
     (app (ad_x 1) tl)). intro. split; assumption.
	simpl in |- *. inversion H2. apply (rec_st d (M2 prec_list (union_mpl_0 ad_z a0 m) m0) (ad_x 1) tl l). simpl in |- *. simpl in H7. assumption. assumption. cut
  (state_reconnait d (union_mpl_0 (ad_x p) a0 (M2 prec_list m m0))
     (app (ad_x 1) tl)). intro. split; assumption.
	induction  p as [p Hrecp| p Hrecp| ]. simpl in |- *. cut (state_reconnait d (union_mpl_0 (ad_x p) a0 m0) (app ad_z tl)).
	intro. inversion H3. apply (rec_st d (M2 prec_list m (union_mpl_0 (ad_x p) a0 m0)) (ad_x 1) tl l). simpl in |- *. simpl in H8. assumption. assumption.
	cut
  (state_reconnait d (union_mpl m0 (M1 prec_list (ad_x p) a0)) (app ad_z tl) /\
   state_reconnait d (union_mpl (M1 prec_list (ad_x p) a0) m0) (app ad_z tl)).
	intro. elim H3. intros. induction  m0 as [| a a1| m0_1 Hrecm0_1 m0_0 Hrecm0_0]; assumption. apply (H0 (M1 prec_list (ad_x p) a0) d ad_z tl). exact (mpl_compat_6 m m0 a0 p H1). inversion H2. simpl in H7.
	exact (rec_st d m0 ad_z tl l H7 H8). simpl in |- *. inversion H2. apply (rec_st d (M2 prec_list (union_mpl_0 (ad_x p) a0 m) m0) (ad_x 1) tl l). simpl in |- *. simpl in H7. assumption.
	assumption. simpl in |- *. cut (state_reconnait d (union_mpl_0 ad_z a0 m0) (app ad_z tl)).
	intro. inversion H3. apply (rec_st d (M2 prec_list m (union_mpl_0 ad_z a0 m0)) (ad_x 1) tl l). simpl in |- *. simpl in H8. assumption. assumption. cut
  (state_reconnait d (union_mpl m0 (M1 prec_list ad_z a0)) (app ad_z tl) /\
   state_reconnait d (union_mpl (M1 prec_list ad_z a0) m0) (app ad_z tl)). intro. elim H3. intros. induction  m0 as [| a a1| m0_1 Hrecm0_1 m0_0 Hrecm0_0]; assumption.
	apply (H0 (M1 prec_list ad_z a0) d ad_z tl). exact (mpl_compat_4 m m0 a0 H1).
	inversion H2. simpl in H7. exact (rec_st d m0 ad_z tl l H7 H8). simpl in |- *. cut (mpl_compat m s1_1).
	cut (mpl_compat m0 s1_0). intros. induction  c as [| p]. cut
  (state_reconnait d (union_mpl m s1_1) (app ad_z tl) /\
   state_reconnait d (union_mpl s1_1 m) (app ad_z tl)). intro. elim H5.
	intros. split. inversion H6. apply
  (rec_st d (M2 prec_list (union_mpl m s1_1) (union_mpl m0 s1_0)) ad_z tl l). simpl in |- *. assumption. assumption. inversion H7. apply
  (rec_st d (M2 prec_list (union_mpl s1_1 m) (union_mpl s1_0 m0)) ad_z tl l). simpl in |- *. assumption. assumption.
	apply (H s1_1 d ad_z tl H4). inversion H2. simpl in H9. exact (rec_st d m ad_z tl l H9 H10).
	induction  p as [p Hrecp| p Hrecp| ]. cut
  (state_reconnait d (union_mpl m0 s1_0) (app (ad_x p) tl) /\
   state_reconnait d (union_mpl s1_0 m0) (app (ad_x p) tl)). intro. elim H5. intros. 
	split. inversion H6. apply
  (rec_st d (M2 prec_list (union_mpl m s1_1) (union_mpl m0 s1_0))
     (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption. inversion H7. apply
  (rec_st d (M2 prec_list (union_mpl s1_1 m) (union_mpl s1_0 m0))
     (ad_x (xI p)) tl l). simpl in |- *.
	assumption. assumption. apply (H0 s1_0 d (ad_x p) tl H3). inversion H2. simpl in H9.
	exact (rec_st d m0 (ad_x p) tl l H9 H10). cut
  (state_reconnait d (union_mpl m s1_1) (app (ad_x p) tl) /\
   state_reconnait d (union_mpl s1_1 m) (app (ad_x p) tl)). intro.
	elim H5. intros. split. inversion H6. apply
  (rec_st d (M2 prec_list (union_mpl m s1_1) (union_mpl m0 s1_0))
     (ad_x (xO p)) tl l). simpl in |- *. assumption. assumption. inversion H7.
	apply
  (rec_st d (M2 prec_list (union_mpl s1_1 m) (union_mpl s1_0 m0))
     (ad_x (xO p)) tl l).
	simpl in |- *. assumption. assumption. apply (H s1_1 d (ad_x p) tl H4). inversion H2. simpl in H9.
	exact (rec_st d m (ad_x p) tl l H9 H10). cut
  (state_reconnait d (union_mpl m0 s1_0) (app ad_z tl) /\
   state_reconnait d (union_mpl s1_0 m0) (app ad_z tl)). intro. elim H5. intros. split.
	inversion H6. apply
  (rec_st d (M2 prec_list (union_mpl m s1_1) (union_mpl m0 s1_0)) 
     (ad_x 1) tl l). simpl in |- *. assumption. assumption. inversion H7. apply
  (rec_st d (M2 prec_list (union_mpl s1_1 m) (union_mpl s1_0 m0)) 
     (ad_x 1) tl l). simpl in |- *. assumption. assumption.
	apply (H0 s1_0 d ad_z tl H3). inversion H2. simpl in H9. exact (rec_st d m0 ad_z tl l H9 H10).
...
Proof.
	unfold union_s_rpl_def in |- *. intros. simpl in H0. elim (bool_is_true_or_false (ad_eq a1 a)); intro; rewrite H1 in H0. inversion H0. simpl in H6. elim (bool_is_true_or_false (ad_eq a1 c)); intro; rewrite H8 in H6. inversion H6. rewrite <- H10 in H7.
	cut (liste_reconnait d pl tl \/ liste_reconnait d a0 tl). intro. elim H9; intro.
	left. rewrite (ad_eq_complete a1 c H8). apply (rec_st d (M1 prec_list c pl) c tl pl).
	simpl in |- *. rewrite (ad_eq_correct c). trivial. assumption. right. rewrite <- (ad_eq_complete a1 a H1). rewrite (ad_eq_complete a1 c H8). apply (rec_st d (M1 prec_list c a0) c tl a0).
	simpl in |- *. rewrite (ad_eq_correct c). trivial. trivial. apply (union_pl_r d pl a0 tl).
	apply (H c pl a0). simpl in |- *. rewrite H8. trivial. simpl in |- *. rewrite <- (ad_eq_complete a1 a H1). rewrite H8. trivial.  assumption. inversion H6.
	elim (ad_sum (ad_xor a a1)); intro y. elim y. intros x y0. rewrite y0 in H0. inversion H0.
	rewrite (MapPut1_semantics prec_list x a a1 a0 pl y0 c) in H6.
	elim (bool_is_true_or_false (ad_eq a c)); intro; rewrite H8 in H6. right. inversion H6.
	apply (rec_st d (M1 prec_list a l) c tl l). simpl in |- *. rewrite H8. trivial. trivial.
	elim (bool_is_true_or_false (ad_eq a1 c)); intro; rewrite H9 in H6. inversion H6.
	left. apply (rec_st d (M1 prec_list a1 l) c tl l). simpl in |- *. rewrite H9. trivial.
	trivial. inversion H6. rewrite (ad_xor_comm a a1) in y. rewrite (ad_xor_eq_true a1 a y) in H1. inversion H1.
Qed.
...
Proof.
	...
	cut
  (state_reconnait d (M1 prec_list ad_z pl) (app ad_z tl) \/
   state_reconnait d m (app ad_z tl)). intro. elim H3. intro. left. trivial. intro. right. inversion H4.
	apply (rec_st d (M2 prec_list m m0) ad_z tl l). simpl in |- *. assumption. assumption.
	apply (H d ad_z pl ad_z tl). apply (mpl_compat_sym m (M1 prec_list ad_z pl)).
	exact
  (mpl_compat_3 m m0 pl
     (mpl_compat_sym (M1 prec_list ad_z pl) (M2 prec_list m m0) H1)).
	inversion H2. simpl in H7. exact (rec_st d (union_mpl_0 ad_z pl m) ad_z tl l H7 H8).
	induction  p as [p Hrecp| p Hrecp| ]. simpl in H2. inversion H2. right. apply (rec_st d (M2 prec_list m m0) (ad_x (xI p)) tl l). simpl in |- *. simpl in H7. assumption. assumption. simpl in H2.
	cut
  (state_reconnait d (M1 prec_list ad_z pl) (app (ad_x p) tl) \/
   state_reconnait d m (app (ad_x p) tl)). intro. elim H3. intro. inversion H4. simpl in H9. inversion H9.
	intro. right. inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x (xO p)) tl l).
	simpl in |- *. assumption. assumption. apply (H d ad_z pl (ad_x p) tl). apply (mpl_compat_sym m (M1 prec_list ad_z pl)). exact
  (mpl_compat_3 m m0 pl
     (mpl_compat_sym (M1 prec_list ad_z pl) (M2 prec_list m m0) H1)). inversion H2. simpl in H7. exact (rec_st d (union_mpl_0 ad_z pl m) (ad_x p) tl l H7 H8). simpl in H2. inversion H2. right. simpl in H7.
	apply (rec_st d (M2 prec_list m m0) (ad_x 1) tl l). simpl in |- *. assumption. assumption.
	induction  p as [p Hrecp| p Hrecp| ]. induction  c as [| p0]. simpl in H2. inversion H2. right. apply (rec_st d (M2 prec_list m m0) ad_z tl l). simpl in |- *. simpl in H7. assumption. assumption.
	simpl in H2. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. cut
  (state_reconnait d (M1 prec_list (ad_x p) pl) (app (ad_x p0) tl) \/
   state_reconnait d m0 (app (ad_x p0) tl)). intro. elim H3; intro.
	inversion H4. left. apply (rec_st d (M1 prec_list (ad_x (xI p)) pl) (ad_x (xI p0)) tl l).
	simpl in |- *. simpl in H9. assumption. assumption. right. inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x (xI p0)) tl l). simpl in |- *. assumption. assumption.
	apply (H0 d (ad_x p) pl (ad_x p0) tl). apply (mpl_compat_sym m0 (M1 prec_list (ad_x p) pl)).
	apply (mpl_compat_6 m m0 pl p). exact (mpl_compat_sym _ _ H1). inversion H2.
	simpl in H7. simpl in |- *. apply (rec_st d (union_mpl_0 (ad_x p) pl m0) (ad_x p0) tl l).
	assumption. assumption. inversion H2. simpl in H7. right. apply (rec_st d (M2 prec_list m m0) (ad_x (xO p0)) tl l). simpl in |- *. assumption. assumption. simpl in H2.
	cut
  (state_reconnait d (M1 prec_list (ad_x p) pl) (app ad_z tl) \/
   state_reconnait d m0 (app ad_z tl)). intro. elim H3; intro. left. inversion H4. simpl in H9. inversion H9.
	right. inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x 1) tl l). simpl in |- *.
	assumption. assumption. apply (H0 d (ad_x p) pl ad_z tl). apply (mpl_compat_sym m0 (M1 prec_list (ad_x p) pl)). apply (mpl_compat_6 m m0 pl p). exact
  (mpl_compat_sym (M1 prec_list (ad_x (xI p)) pl) (M2 prec_list m m0) H1). inversion H2. simpl in H7.
	exact (rec_st d (union_mpl_0 (ad_x p) pl m0) ad_z tl l H7 H8). induction  c as [| p0]. simpl in H2.
	cut
  (state_reconnait d (M1 prec_list (ad_x p) pl) (app ad_z tl) \/
   state_reconnait d m (app ad_z tl)). intro. elim H3; intros. inversion H4. inversion H9. right. inversion H4.
	apply (rec_st d (M2 prec_list m m0) ad_z tl l). simpl in |- *. assumption. assumption.
	apply (H d (ad_x p) pl ad_z tl). apply (mpl_compat_sym m (M1 prec_list (ad_x p) pl)).
	apply (mpl_compat_5 m m0 pl p). exact
  (mpl_compat_sym (M1 prec_list (ad_x (xO p)) pl) (M2 prec_list m m0) H1). inversion H2. apply (rec_st d (union_mpl_0 (ad_x p) pl m) ad_z tl l).
	simpl in |- *. simpl in H7. assumption. assumption. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. simpl in H2. inversion H2. right.
	apply (rec_st d (M2 prec_list m m0) (ad_x (xI p0)) tl l). simpl in |- *. simpl in H7. assumption.
	assumption. simpl in H2. cut
  (state_reconnait d (M1 prec_list (ad_x p) pl) (app (ad_x p0) tl) \/
   state_reconnait d m (app (ad_x p0) tl)). intro. elim H3; intro. inversion H4. simpl in H9.
	left. apply (rec_st d (M1 prec_list (ad_x (xO p)) pl) (ad_x (xO p0)) tl l). simpl in |- *. assumption.
	assumption. right. inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x (xO p0)) tl l).
	simpl in |- *. assumption. assumption. apply (H d (ad_x p) pl (ad_x p0) tl). apply (mpl_compat_sym m (M1 prec_list (ad_x p) pl)). exact (mpl_compat_5 m m0 pl p (mpl_compat_sym _ _ H1)).
	inversion H2. apply (rec_st d (union_mpl_0 (ad_x p) pl m) (ad_x p0) tl l). simpl in H7.
	assumption. assumption. simpl in H2. inversion H2. right. apply (rec_st d (M2 prec_list m m0) (ad_x 1) tl l). simpl in |- *. simpl in H7. assumption. assumption. induction  c as [| p].
	simpl in H2. right. inversion H2. apply (rec_st d (M2 prec_list m m0) ad_z tl l).
	simpl in |- *. simpl in H7. assumption. assumption. induction  p as [p Hrecp| p Hrecp| ]. simpl in H2.
	cut
  (state_reconnait d (M1 prec_list ad_z pl) (app (ad_x p) tl) \/
   state_reconnait d m0 (app (ad_x p) tl)). intro. elim H3; intros. left. inversion H4. simpl in H9. inversion H9.
	right. inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x (xI p)) tl l). simpl in |- *.
	assumption. assumption. apply (H0 d ad_z pl (ad_x p) tl). apply (mpl_compat_sym m0 (M1 prec_list ad_z pl)). exact (mpl_compat_4 m m0 pl (mpl_compat_sym _ _ H1)). 
	inversion H2. apply (rec_st d (union_mpl_0 ad_z pl m0) (ad_x p) tl l). simpl in |- *. simpl in H7.
	assumption. assumption. simpl in H2. inversion H2. right. apply (rec_st d (M2 prec_list m m0) (ad_x (xO p)) tl l). simpl in |- *. simpl in H7. assumption. assumption. simpl in H2.
	cut
  (state_reconnait d (M1 prec_list ad_z pl) (app ad_z tl) \/
   state_reconnait d m0 (app ad_z tl)). intro. elim H3; intros. left. inversion H4. apply (rec_st d (M1 prec_list (ad_x 1) pl) (ad_x 1) tl l). simpl in |- *. assumption. assumption. right.
	inversion H4. apply (rec_st d (M2 prec_list m m0) (ad_x 1) tl l). simpl in |- *.
	assumption. assumption. apply (H0 d ad_z pl ad_z tl). apply (mpl_compat_sym m0 (M1 prec_list ad_z pl)). exact (mpl_compat_4 m m0 pl (mpl_compat_sym _ _ H1)).
	inversion H2. simpl in |- *. simpl in H7. apply (rec_st d (union_mpl_0 ad_z pl m0) ad_z tl l).
...
Proof.
  ...
  intro. right. trivial. left. trivial. cut (mpl_compat m s1_1).
	...
	cut
  (state_reconnait d (union_mpl m s1_1) (app ad_z tl) \/
   state_reconnait d (union_mpl s1_1 m) (app ad_z tl)). intro. elim (H s1_1 d ad_z tl H4 H5); intro. inversion H6. left. apply (rec_st d (M2 prec_list m m0) ad_z tl l).
	simpl in |- *. assumption. assumption. right. inversion H6. apply (rec_st d (M2 prec_list s1_1 s1_0) ad_z tl l). simpl in |- *. assumption. assumption. elim H2.
	intro. inversion H5. left. apply (rec_st d (union_mpl m s1_1) ad_z tl l).
	simpl in H10. assumption. assumption. intro. right. inversion H5.
	apply (rec_st d (union_mpl s1_1 m) ad_z tl l). simpl in H10. assumption.
	assumption.  induction  p as [p Hrecp| p Hrecp| ]. simpl in H2. clear Hrecp. clear Hrecs1_1. 
	clear Hrecs1_0. cut
  (state_reconnait d (union_mpl m0 s1_0) (app (ad_x p) tl) \/
   state_reconnait d (union_mpl s1_0 m0) (app (ad_x p) tl)). intro. 
	elim (H0 s1_0 d (ad_x p) tl H3 H5). intro. left. inversion H6.
	apply (rec_st d (M2 prec_list m m0) (ad_x (xI p)) tl l). simpl in |- *. assumption.
	assumption. intro. inversion H6. right. apply (rec_st d (M2 prec_list s1_1 s1_0) (ad_x (xI p)) tl l). simpl in |- *. assumption. assumption. elim H2; intro.
	left. inversion H5. apply (rec_st d (union_mpl m0 s1_0) (ad_x p) tl l).
	simpl in H10. assumption. assumption. inversion H5. right.
	apply (rec_st d (union_mpl s1_0 m0) (ad_x p) tl l). simpl in H10. simpl in |- *. 
	assumption. assumption. clear Hrecp. clear Hrecs1_1. clear Hrecs1_0.
	simpl in H2. cut
  (state_reconnait d (union_mpl m s1_1) (app (ad_x p) tl) \/
   state_reconnait d (union_mpl s1_1 m) (app (ad_x p) tl)). intro.
	elim (H s1_1 d (ad_x p) tl H4 H5). intro. left. inversion H6.
	apply (rec_st d (M2 prec_list m m0) (ad_x (xO p)) tl l). simpl in |- *.
	assumption.  assumption. intro. right. inversion H6.
	apply (rec_st d (M2 prec_list s1_1 s1_0) (ad_x (xO p)) tl l). simpl in |- *.
	assumption. assumption. elim H2; intro; inversion H5. left.
	apply (rec_st d (union_mpl m s1_1) (ad_x p) tl l). simpl in H10. assumption.
	assumption. right. apply (rec_st d (union_mpl s1_1 m) (ad_x p) tl l). 
	simpl in H10. assumption. assumption. simpl in H2. cut
  (state_reconnait d (union_mpl m0 s1_0) (app ad_z tl) \/
   state_reconnait d (union_mpl s1_0 m0) (app ad_z tl)). intro. elim (H0 s1_0 d ad_z tl H3 H5); intro. inversion H6.
	left. apply (rec_st d (M2 prec_list m m0) (ad_x 1) tl l). simpl in |- *. assumption.
	assumption. inversion H6. right. apply (rec_st d (M2 prec_list s1_1 s1_0) (ad_x 1) tl l). simpl in |- *. assumption. assumption. elim H2; intro; inversion H5.
	left. simpl in H10. apply (rec_st d (union_mpl m0 s1_0) ad_z tl l). assumption.
	assumption. right. apply (rec_st d (union_mpl s1_0 m0) ad_z tl l). simpl in H10.
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a0 a)); intros.
	rewrite (ad_eq_complete a0 a H1) in H0. rewrite H0 in e.
...
Lemma new_state_ins_d :
 ...
 MapGet state d a = NONE state -> reconnaissance (MapPut state d a s) a0 t.
Proof.
...
Definition new_state_insr_def_dta (d0 : preDTA) (a0 : ad) 
  ...
  MapGet state d a = NONE state -> a <> a0 -> reconnaissance d a0 t0.
...
Definition new_state_insr_def_st (d0 : preDTA) (s0 : state) 
  ...
  d0 = MapPut state d a s ->
  MapGet state d a = NONE state -> state_reconnait d s0 t0.
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a0 a)); intro.
	elim (H3 (ad_eq_complete _ _ H4)). rewrite H4 in e.
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a x0)); intro.
	elim (H12 (ad_eq_complete _ _ H13)). rewrite H13 in H7.
...
Proof.
	...
	cut (MapGet state d0 x0 = SOME state x). cut (prec_occur x2 a).
	intros. elim (H1 x0 x x1 x2 a H14 H11 H13). intros. elim (classic (a0 = a)). intro. rewrite <- H16 in H15. rewrite H15 in H3. inversion H3.
	intro. assumption. elim H12. intros. exact (prec_occur_1 a la ls x2 H13). rewrite H2 in H9. rewrite (MapPut_semantics state d0 a0 s) in H9. elim (bool_is_true_or_false (ad_eq a0 x0)). intro. elim H12.
	intros. elim (H15 (ad_eq_complete _ _ H13)). intros. rewrite H13 in H9.
	...
	elim (bool_is_true_or_false (ad_eq a0 x0)); intro. elim (H14 (ad_eq_complete _ _ H15)). rewrite H15 in H9. split with x. 
...
Proof.
	...
	split with x1. split with x2. split. rewrite H1 in H8. rewrite (MapPut_semantics state d0 a0 s) in H8. elim (bool_is_true_or_false (ad_eq a0 x0)); intro. elim (H13 (ad_eq_complete _ _ H14)). 
...
Lemma insert_ostate_0 :
 ...
 a <> a0 ->
 (reconnaissance d a0 t <->
  reconnaissance (insert_ostate d a (SOME state s)) a0 t).
Proof.
...
Lemma insert_ostate_1 :
 ...
 MapGet state (u_merge d0 d1) (uad_conv_0 a0) = SOME state s0' ->
 MapGet state (u_merge d0 d1) (uad_conv_1 a1) = SOME state s1' ->
 preDTA_ref_ok (u_merge d0 d1) ->
 MapGet state (u_merge d0 d1) a = NONE state ->
 a <> uad_conv_0 a0 ->
 (reconnaissance d0 a0 t <->
  reconnaissance (insert_ostate (u_merge d0 d1) a (SOME state s))
    (uad_conv_0 a0) t).
Proof.
	intros. cut
...
Lemma insert_ostate_3 :
 ...
 (reconnaissance (insert_ostate (u_merge d0 d1) a (SOME state s))
    (uad_conv_0 a0) t <->
  state_reconnait (insert_ostate (u_merge d0 d1) a (SOME state s)) s0' t).
Proof.
	intros. split. intro. inversion H6. unfold insert_ostate in H7.
	rewrite (MapPut_semantics state (u_merge d0 d1) a s) in H7.
	elim (bool_is_true_or_false (ad_eq a (uad_conv_0 a0))); intro.
	elim (H5 (ad_eq_complete _ _ H12)). rewrite H12 in H7. induction  t as (a3, t).
	inversion H8. apply (rec_st (insert_ostate (u_merge d0 d1) a (SOME state s)) s0' a3 t l). rewrite H7 in H1. inversion H1.
	rewrite <- H20. assumption. assumption. intro.
	apply
  (rec_dta (insert_ostate (u_merge d0 d1) a (SOME state s)) 
     (uad_conv_0 a0) t s0'). unfold insert_ostate in |- *.
	rewrite (MapPut_semantics state (u_merge d0 d1) a s).
	elim (bool_is_true_or_false (ad_eq a (uad_conv_0 a0))); intro.
	elim (H5 (ad_eq_complete _ _ H7)). rewrite H7. assumption.
	assumption.
Qed.
...
Lemma insert_ostate_4 :
 ...
 (reconnaissance (insert_ostate (u_merge d0 d1) a (SOME state s))
    (uad_conv_1 a1) t <->
  state_reconnait (insert_ostate (u_merge d0 d1) a (SOME state s)) s1' t).
Proof.
	intros. split. intro. inversion H6. unfold insert_ostate in H7.
	rewrite (MapPut_semantics state (u_merge d0 d1) a s) in H7.
	elim (bool_is_true_or_false (ad_eq a (uad_conv_1 a1))); intro.
	elim (H5 (ad_eq_complete _ _ H12)). rewrite H12 in H7. induction  t as (a3, t).
	inversion H8. apply (rec_st (insert_ostate (u_merge d0 d1) a (SOME state s)) s1' a3 t l). rewrite H7 in H2. inversion H2.
	rewrite <- H20. assumption. assumption. intro.
	apply
  (rec_dta (insert_ostate (u_merge d0 d1) a (SOME state s)) 
...
Lemma insert_ostate_5 :
 ...
 a <> uad_conv_1 a1 ->
 (state_reconnait
    (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1')))
    (union_mpl s0' s1') t <->
  state_reconnait
    (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1'))) s0' t \/
  state_reconnait
    (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1'))) s1' t).
Proof.
	intros. exact
  (union_state s0' s1'
     (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1'))) t H).
Qed.
...
Lemma insert_ostate_6 :
 ...
 (state_reconnait
    ...
    (union_mpl s0' s1') t <->
  reconnaissance
    (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1'))) a t).
Proof.
	intros. split. intro. apply
  (rec_dta (insert_ostate (u_merge d0 d1) a (SOME state (union_mpl s0' s1')))
     a t (union_mpl s0' s1')).
	unfold insert_ostate in |- *. rewrite (MapPut_semantics state (u_merge d0 d1) a (union_mpl s0' s1')). rewrite (ad_eq_correct a). trivial. assumption.
	intro. inversion H8. unfold insert_ostate in H9. rewrite (MapPut_semantics state (u_merge d0 d1) a (union_mpl s0' s1')) in H9. rewrite (ad_eq_correct a) in H9. inversion H9. rewrite <- H15 in H10. assumption.
Qed.
...
Definition union_opt_state (o0 o1 : option state) : 
  ...
  | NONE, SOME s1 => SOME state s1
  | SOME s0, NONE => SOME state s0
  | SOME s0, SOME s1 => SOME state (union_mpl s0 s1)
  end.
