...
Lemma domain_equal_mapget :
 ...
 MapGet A m0 a = SOME A x -> exists y : B, MapGet B m1 a = SOME B y.
Proof.
	simple induction m0. intros. inversion H0. intros. induction  m1 as [| a2 a3| m1_1 Hrecm1_1 m1_0 Hrecm1_0]. inversion H.
	simpl in H0. simpl in H. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H1 in H0. inversion H0. simpl in |- *. rewrite <- H. rewrite H1.
...
Fixpoint lem (m0 m1 : Map bool) {struct m1} : Prop :=
  ...
  | M0, M2 _ _ => False
  | M1 _ _, M0 => False
  | M1 a b, M1 a' b' => if ad_eq a a' then leb b b' else False
  | M1 _ _, M2 _ _ => False
  | M2 _ _, M0 => False
  | M2 _ _, M1 _ _ => False
...
Proof.
	...
	rewrite (ad_eq_correct a). exact (leb_reflexive a0). intros.
...
Proof.
	...
	intros. simpl in H. elim (bool_is_true_or_false (ad_eq a a1)).
	intros. rewrite H1 in H. simpl in H0. rewrite (ad_eq_comm a1 a) in H0. rewrite H1 in H0. rewrite (ad_eq_complete _ _ H1). rewrite (leb_antisymmetric _ _ H H0). reflexivity. intros. rewrite H1 in H.
...
Proof.
	...
	elim H0. intros. simpl in H. simpl in H0. elim (bool_is_true_or_false (ad_eq a a1)); intro. rewrite H1 in H. elim (bool_is_true_or_false (ad_eq a1 a3)); intro. rewrite (ad_eq_complete _ _ H1). rewrite (ad_eq_complete _ _ H2). rewrite H2 in H0. simpl in |- *. rewrite (ad_eq_correct a3). exact (leb_transitive _ _ _ H H0). rewrite H2 in H0.
...
Proof.
	...
	rewrite (ad_eq_correct a1). elim (bool_is_true_or_false a2); intro; rewrite H0; exact I. elim H. intros. induction  x as [| a a0| x2 Hrecx2 x3 Hrecx3]. elim H. elim H. simpl in |- *.
...
Lemma map_mini_mapget_false :
 ...
 MapGet bool (map_mini A x) a = SOME bool b -> b = false.
Proof.
	intro. simple induction x. intros. inversion H. intros. simpl in H.
	elim (bool_is_true_or_false (ad_eq a a1)); intro; rewrite H0 in H. inversion H. reflexivity. inversion H. intros. induction  a as [| p].
...
Lemma map_mini_mapget_true :
 ...
 MapGet bool (map_mini A x) a = SOME bool true -> False.
Proof.
...
Proof.
	...
	induction  x as [| a3 a4| x4 Hrecx1 x5 Hrecx0]. elim H20. induction  x1 as [| a5 a6| x1_1 Hrecx1_1 x1_0 Hrecx1_0]. elim H17. simpl in H13. simpl in H21. elim (bool_is_true_or_false (ad_eq a3 a1)); intro; rewrite H22 in H13. elim (bool_is_true_or_false (ad_eq a5 a3)); intro; rewrite H23 in H21. inversion H6. inversion H28. rewrite (ad_eq_complete _ _ H22) in H26.
	rewrite (ad_eq_complete _ _ H23) in H30. induction  a6 as [| ]. induction  a4 as [| ].
	...
	simpl in H22. elim (bool_is_true_or_false (ad_eq a3 a1)); intro; rewrite H31 in H14. elim (bool_is_true_or_false (ad_eq a5 a3)); intro; rewrite H32 in H22. inversion H7. inversion H37. rewrite (ad_eq_complete _ _ H31) in H35. rewrite (ad_eq_complete _ _ H32) in H40. induction  a6 as [| ]. induction  a4 as [| ].
	...
	induction  x1 as [| a5 a6| x1_1 Hrecx1_1 x1_0 Hrecx1_0]. elim H26. simpl in H14. simpl in H22. elim (bool_is_true_or_false (ad_eq a3 a1)); intro; rewrite H31 in H14.
	elim (bool_is_true_or_false (ad_eq a5 a3)); intro; rewrite H32 in H22.
	inversion H7. inversion H37. rewrite (ad_eq_complete _ _ H31) in H35.
	rewrite (ad_eq_complete _ _ H32) in H40. induction  a6 as [| ]. induction  a4 as [| ].
	elim H40. reflexivity. elim H22. induction  a4 as [| ]. induction  a2 as [| ]. elim H35.
...
Fixpoint eqm_bool (x y : Map bool) {struct y} : bool :=
  ...
  | M0, M2 _ _ => false
  | M1 _ _, M0 => false
  | M1 a b, M1 c d => ad_eq a c && eq_bool b d
  | M1 _ _, M2 _ _ => false
  | M2 _ _, M0 => false
  | M2 _ _, M1 _ _ => false
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a a1)); intro; rewrite H0 in H.
	rewrite (ad_eq_complete _ _ H0). elim (bool_is_true_or_false (eq_bool a0 a2)); intro; rewrite H1 in H. rewrite (eq_bool_equal _ _ H1).
...
Proof.
	simple induction x. reflexivity. intros. simpl in |- *. rewrite (ad_eq_correct a).
...
Lemma lem_get_leb :
 ...
 lem m0 m1 ->
 MapGet bool m0 a = SOME bool b0 ->
 MapGet bool m1 a = SOME bool b1 -> leb b0 b1.
Proof.
	simple induction m0. intros. inversion H0. simple induction m1. intros. inversion H1.
	intros. simpl in H. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H2 in H. simpl in H0. simpl in H1. elim (bool_is_true_or_false (ad_eq a a3)); intro; rewrite H3 in H0. inversion H0. elim (bool_is_true_or_false (ad_eq a1 a3)); intros; rewrite H4 in H1.
...
Proof.
	...
	simpl in |- *. simpl in H. elim (bool_is_true_or_false (ad_eq a a1)); intros.
	exact (ad_eq_complete _ _ H0). rewrite H0 in H. elim H. intros.
