...
Fixpoint prec_list_kill (m : Map bool) (p : prec_list) {struct p} :
 option prec_list :=
  match p with
  | prec_empty => SOME prec_list prec_empty
  | prec_cons a la ls =>
      match ls with
      | prec_empty =>
          ...
          | SOME b =>
              ...
              then
               ...
               | NONE => NONE prec_list
               | SOME la' => SOME prec_list (prec_cons a la' prec_empty)
               end
              else NONE prec_list
          end
      | prec_cons _ _ _ =>
          ...
          | SOME b =>
              ...
              then
               ...
               | NONE, NONE => NONE prec_list
               | NONE, SOME ls' => SOME prec_list ls'
               | SOME la', NONE =>
                   SOME prec_list (prec_cons a la' prec_empty)
               | SOME la', SOME ls' => SOME prec_list (prec_cons a la' ls')
               end
...
Fixpoint states_kill_aux (m : Map bool) (s : state) {struct s} : state :=
  ...
  | M2 s0 s1 =>
      ...
      | M0, M0 => M0 prec_list
      | M0, M1 a p => M1 prec_list (ad_double_plus_un a) p
      | M1 a p, M0 => M1 prec_list (ad_double a) p
...
Fixpoint preDTA_kill (m : Map bool) (d : preDTA) {struct d} : preDTA :=
  ...
  | M2 d0 d1 =>
      ...
      | M0, M0 => M0 state
      | M0, M1 a s' => M1 state (ad_double_plus_un a) s'
      | M1 a s', M0 => M1 state (ad_double a) s'
...
Definition DTA_simpl (d : DTA) : DTA :=
  ...
  | dta p a =>
      ...
      | NONE => dta (M1 state ad_z (M0 prec_list)) ad_z
      | _ => dta p a
      end
  end.
...
Lemma pl_kill_0 :
 ...
 pl_path_true plp m ->
 exists pl : prec_list,
   prec_list_kill m p = SOME prec_list pl /\ pl_path_incl plp pl.
Proof.
...
Lemma pl_kill_prec_empty :
 ...
 prec_list_kill m p = SOME prec_list prec_empty -> p = prec_empty.
Proof.
...
Lemma st_kill_0 :
 ...
 MapGet prec_list (states_kill_aux m s) a = SOME prec_list p'.
Proof.
	simple induction s. intros. inversion H0. intros. simpl in H0. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H1 in H0.
	...
	induction  a as [| p0]. cut (MapGet prec_list (states_kill_aux m1 m) ad_z = SOME prec_list p'). intro. rewrite H3 in H8. inversion H8.
	exact (H _ _ _ _ H1 H2). induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. rewrite <- (H0 _ _ _ _ H1 H2). rewrite H7. simpl in |- *. elim (bool_is_true_or_false (ad_eq x (ad_x p0))). intro. rewrite H8. rewrite (ad_eq_complete _ _ H8). simpl in |- *. rewrite (aux_ad_eq_1_0 p0). reflexivity. intro.
	rewrite H8. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x (xI p0)))). intro. rewrite (ad_double_plus_un_inv_xI _ _ (ad_eq_complete _ _ H9)) in H8. rewrite (ad_eq_correct (ad_x p0)) in H8. inversion H8. intro. rewrite H9. reflexivity.
	rewrite <- (H _ _ _ _ H1 H2). rewrite H3. induction  x as [| p1]. simpl in |- *.
	reflexivity. simpl in |- *. reflexivity. rewrite <- (H0 _ _ _ _ H1 H2).
	simpl in |- *. rewrite H7. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x 1))); intro;
  rewrite H8. elim (bool_is_true_or_false (ad_eq x ad_z)); intro; rewrite H9.
	reflexivity. rewrite (ad_double_plus_un_inv_xH _ (ad_eq_complete _ _ H8)) in H9. rewrite (ad_eq_correct ad_z) in H9. inversion H9.
	elim (bool_is_true_or_false (ad_eq x ad_z)); intro; rewrite H9.
	rewrite (ad_eq_complete _ _ H9) in H8. inversion H8. 
	...
	elim H4. intros. elim H5. intros. rewrite H6. elim (map_sum prec_list (states_kill_aux m1 m0)); intro. rewrite H7. induction  a as [| p0]. rewrite <- (H _ _ _ _ H1 H2). rewrite H6. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double x) ad_z)); intro; rewrite H8. elim (bool_is_true_or_false (ad_eq x ad_z)); intro; rewrite H9. reflexivity. rewrite (ad_double_inv_ad_z _ (ad_eq_complete _ _ H8)) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x ad_z)); intro; rewrite H9.
	rewrite (ad_eq_complete _ _ H9) in H8. inversion H8.
	...
	elim (bool_is_true_or_false (ad_eq (ad_double x) (ad_x (xO p0)))); intro;
  rewrite H8. elim (bool_is_true_or_false (ad_eq x (ad_x p0))); intro; rewrite H9. reflexivity. rewrite (ad_double_inv_xO _ _ (ad_eq_complete _ _ H8)) in H9. rewrite (ad_eq_correct (ad_x p0)) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x (ad_x p0))); intro; rewrite H9. rewrite (ad_eq_complete _ _ H9) in H8. simpl in H8. rewrite (aux_ad_eq_1_0 p0) in H8. inversion H8.
...
Lemma st_kill_1 :
 ...
 MapGet prec_list s a = SOME prec_list p ->
 exists s' : state,
   states_kill m s = SOME state s' /\
   MapGet prec_list s' a = SOME prec_list p'.
Proof.
	intros. unfold states_kill in |- *. elim (map_sum prec_list (states_kill_aux m s)); intros. cut (MapGet prec_list (states_kill_aux m s) a = SOME prec_list p'). intro.
...
Proof.
	...
	elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H0 in H. simpl in |- *. rewrite H0. inversion H. split with a0.
	...
	intros. rewrite H6 in H1. induction  a as [| p0]. induction  x as [| p0]; inversion H1. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. elim (H0 m1 (ad_x p0) p).
	intros. elim H7. intros. split with x1. simpl in |- *. split; assumption. simpl in H1. rewrite H6. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x (xI p0))));
  intros; rewrite H7 in H1. inversion H1. rewrite (ad_double_plus_un_inv_xI _ _ (ad_eq_complete _ _ H7)). rewrite (ad_eq_correct (ad_x p0)). reflexivity.
	inversion H1. simpl in H1. induction  x as [| p1]; inversion H1.
	elim (H0 m1 ad_z p). intros. elim H7. intros. split with x1. simpl in |- *. split; assumption. rewrite H6. simpl in |- *. simpl in H1. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x 1))); intros;
  rewrite H7 in H1. inversion H1.
	elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H8. reflexivity. rewrite (ad_double_plus_un_inv_xH _ (ad_eq_complete _ _ H7)) in H8. inversion H8. inversion H1. intros. elim H4. intros. elim H5. intros. rewrite H6 in H1. induction  a as [| p0]. inversion H1. rewrite <- H6 in H1.
	...
	simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double x) ad_z)); intros;
  rewrite H7 in H1. inversion H1. apply (H m1 ad_z p). rewrite H5. simpl in |- *. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H8. rewrite H9. reflexivity.
	rewrite (ad_double_inv_ad_z _ (ad_eq_complete _ _ H7)) in H8.
	inversion H8. inversion H1. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]. induction  x as [| p1]; inversion H1. simpl in H1. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double x) (ad_x (xO p0)))); intros;
  rewrite H7 in H1. inversion H1. apply (H m1 (ad_x p0) p). rewrite H5.
	simpl in |- *. elim (bool_is_true_or_false (ad_eq x (ad_x p0))); intros; rewrite H8. rewrite H9. reflexivity. rewrite (ad_double_inv_xO _ _ (ad_eq_complete _ _ H7)) in H8.
	rewrite (ad_eq_correct (ad_x p0)) in H8. inversion H8.
	inversion H1. induction  x as [| p0]; inversion H1. elim H6; intros; elim H7; intros; elim H8; intros. rewrite H9 in H1.
	induction  a as [| p0]. simpl in |- *. simpl in H1. apply (H m1 ad_z p).
	rewrite H5. simpl in |- *. exact H1. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]; simpl in |- *; simpl in H1. apply (H0 m1 (ad_x p0) p). rewrite H9. simpl in |- *.
	exact H1. apply (H m1 (ad_x p0) p). rewrite H5. simpl in |- *.
	exact H1. apply (H0 m1 ad_z p). rewrite H9. exact H1.
	rewrite H9 in H1. rewrite <- H9 in H1. induction  a as [| p0].
	simpl in H1. simpl in |- *. apply (H m1 ad_z p). rewrite H5.
	simpl in |- *. exact H1. induction  p0 as [p0 Hrecp0| p0 Hrecp0| ]; simpl in |- *; simpl in H1.
	apply (H0 m1 (ad_x p0) p). rewrite H9. rewrite H9 in H1.
	exact H1. apply (H m1 (ad_x p0) p). rewrite H5. simpl in |- *.
	exact H1. apply (H0 m1 ad_z p). rewrite H9. simpl in |- *.
...
Lemma st_kill_3 :
 ...
 s' <> M0 prec_list ->
 exists p : prec_list,
   (exists a : ad, MapGet prec_list s' a = SOME prec_list p).
Proof.
	...
	split with a. simpl in |- *. rewrite (ad_eq_correct a). reflexivity.
	...
	split with a0. split with (ad_double_plus_un x1). simpl in |- *.
	rewrite (ad_eq_correct (ad_double_plus_un x1)). reflexivity.
	...
	split with a0. split with (ad_double x1). simpl in |- *. rewrite (ad_eq_correct (ad_double x1)). reflexivity. elim H9; intros; elim H10; intros; elim H11; intros; rewrite H12 in H.
	...
	intros. split with x3. split with (ad_double_plus_un x4).
	...
	split with (ad_double x6). rewrite <- H16 in H18. simpl in H18. induction  x6 as [| p]; simpl in |- *; exact H18. rewrite H16 in H10.
	exact H10. intro. rewrite <- H16 in H15. inversion H15.
	inversion H1. elim (H x m1); intros. elim H15. intros.
	split with x5. split with (ad_double x6). rewrite <- H16 in H18. simpl in H18. induction  x6 as [| p]; simpl in |- *; exact H18.
	rewrite H16 in H10. exact H10. intro. rewrite <- H16 in H15. inversion H15. inversion H1. elim (H x m1). intros.
	elim H11. intros. split with x3. split with (ad_double x4).
...
Lemma dt_kill_0 :
 ...
 MapGet state (preDTA_kill m d) a = SOME state s'.
Proof.
	simple induction d. intros. inversion H0. intros. simpl in H0.
	elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H1 in H0. simpl in |- *. inversion H0. rewrite H.
	...
	rewrite H7. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x (xI p))));
  intros; rewrite H8.
	elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros; rewrite H9. reflexivity. rewrite (ad_double_plus_un_inv_xI _ _ (ad_eq_complete _ _ H8)) in H9. rewrite (ad_eq_correct (ad_x p)) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros. rewrite H9. rewrite (ad_eq_complete _ _ H9) in H8. simpl in H8. rewrite (aux_ad_eq_1_0 p) in H8. inversion H8. rewrite H9.
	reflexivity. rewrite <- (H _ _ _ _ H1 H2). rewrite H3.
	simpl in |- *. induction  x as [| p0]; reflexivity. rewrite <- (H0 _ _ _ _ H1 H2). rewrite H7. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x 1))); intros;
  rewrite H8. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H9. reflexivity. rewrite (ad_double_plus_un_inv_xH _ (ad_eq_complete _ _ H8)) in H9. rewrite (ad_eq_correct ad_z) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x ad_z)); intro; rewrite H9. rewrite (ad_eq_complete _ _ H9) in H8. simpl in H8. inversion H8. reflexivity.
	...
	rewrite <- (H _ _ _ _ H1 H2). rewrite H6. simpl in |- *.
	elim (bool_is_true_or_false (ad_eq (ad_double x) ad_z)); intros; rewrite H8. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H9. reflexivity. rewrite (ad_double_inv_ad_z _ (ad_eq_complete _ _ H8)) in H9.
	rewrite (ad_eq_correct ad_z) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H9.
	rewrite (ad_eq_complete _ _ H9) in H8. simpl in H8. inversion H8. reflexivity. induction  p as [p Hrecp| p Hrecp| ]. rewrite <- (H0 _ _ _ _ H1 H2).
	rewrite H7. induction  x as [| p0]; reflexivity. rewrite <- (H _ _ _ _ H1 H2). rewrite H6. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double x) (ad_x (xO p)))); intros;
  rewrite H8.
	elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros; rewrite H9. reflexivity. rewrite (ad_double_inv_xO _ _ (ad_eq_complete _ _ H8)) in H9. rewrite (ad_eq_correct (ad_x p)) in H9. inversion H9. elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros; rewrite H9. rewrite (ad_eq_complete _ _ H9) in H8. simpl in H8. rewrite (aux_ad_eq_1_0 p) in H8. inversion H8. reflexivity.
...
Lemma dt_kill_1 :
 ...
 MapGet state (preDTA_kill m d) a = SOME state s ->
 exists s' : state,
   MapGet state d a = SOME state s' /\ states_kill m s' = SOME state s.
Proof.
	simple induction d. intros. inversion H. intros. simpl in H.
	simpl in |- *. elim (option_sum state (states_kill m a0)); intro y.
	elim y. intros x y0. rewrite y0 in H. simpl in H. elim (bool_is_true_or_false (ad_eq a a1)); intros; rewrite H0 in H;
  rewrite H0. inversion H. split with a0. split. reflexivity.
	...
	apply (H0 m1 (ad_x p) s). rewrite H6. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x (xI p))));
  intros; rewrite H7 in H1. inversion H1. elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros; rewrite H8. reflexivity. rewrite (ad_double_plus_un_inv_xI _ _ (ad_eq_complete _ _ H7)) in H8. rewrite (ad_eq_correct (ad_x p)) in H8. inversion H8. inversion H1. induction  x as [| p0]; inversion H1. apply (H0 m1 ad_z s). rewrite H6. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double_plus_un x) (ad_x 1))); intros;
  rewrite H7 in H1. inversion H1. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H8.
	reflexivity. rewrite (ad_double_plus_un_inv_xH _ (ad_eq_complete _ _ H7)) in H8. inversion H8. inversion H1.
	intros. elim H4. intros. elim H5. intros. rewrite H6 in H1.
	induction  a as [| p]. inversion H1. rewrite <- H6 in H1. induction  p as [p Hrecp| p Hrecp| ]; simpl in |- *; simpl in H1. apply (H0 m1 (ad_x p) s). rewrite H6.
	...
	apply (H m1 ad_z s). rewrite H5. simpl in |- *. elim (bool_is_true_or_false (ad_eq (ad_double x) ad_z)); intros;
  rewrite H7 in H1. inversion H1. elim (bool_is_true_or_false (ad_eq x ad_z)); intros; rewrite H8. reflexivity. rewrite (ad_double_inv_ad_z _ (ad_eq_complete _ _ H7)) in H8.
	inversion H8. inversion H1. induction  p as [p Hrecp| p Hrecp| ]; simpl in |- *; simpl in H1.
	induction  x as [| p0]; inversion H1. elim (bool_is_true_or_false (ad_eq (ad_double x) (ad_x (xO p)))); intros;
  rewrite H7 in H1.
	inversion H1. apply (H m1 (ad_x p) s). rewrite H5. simpl in |- *.
	elim (bool_is_true_or_false (ad_eq x (ad_x p))); intros; rewrite H8. assumption. rewrite (ad_double_inv_xO _ _ (ad_eq_complete _ _ H7)) in H8. rewrite (ad_eq_correct (ad_x p)) in H8. inversion H8. inversion H1. induction  x as [| p]; inversion H1. elim H6; intros; elim H7; intros; elim H8; intros; rewrite H9 in H1. rewrite <- H9 in H1. induction  a as [| p].
...
Lemma dt_kill_empty_kill_empty_3 :
 ...
 MapGet state (preDTA_kill (dta_non_empty_states d) d) a = SOME state s ->
 predta_correct_wrt_sign d sigma ->
